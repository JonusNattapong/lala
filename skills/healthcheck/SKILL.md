---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Lala deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Lala cron scheduling for periodic checks, or version status checks on a machine running Lala (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Lala, then align it to a user-defined risk tolerance without breaking access. Use Lala security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Lala changes the host firewall, SSH, or OS updates; it does not.
...
5. Lala gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Lala can access sensitive data.
...
### 2) Run Lala security audits (read-only)

As part of the default read-only checks, run `lala security audit --deep`. Only offer alternatives if the user requests them:

1. `lala security audit` (faster, non-probing)
2. `lala security audit --json` (structured output)

Offer to apply Lala safe defaults (numbered):

1. `lala security audit --fix`

Be explicit that `--fix` only tightens Lala defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Lala version/update status (read-only)

As part of the default read-only checks, run `lala update status`.
...
- Credential hygiene notes (location of Lala creds, prefer disk encryption)
...
After Lala install or first hardening pass, run at least one baseline audit and version check:

- `lala security audit`
- `lala security audit --deep`
- `lala update status`

Ongoing monitoring is recommended. Use the Lala cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `lala cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `lala update status`
...
Before creating, `lala cron list` and match on exact `name`. If found, `lala cron edit <id> ...`.
If not found, `lala cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `lala update status` (preferred for source checkouts and channels)
2. `npm view lala version` (published npm version)

## Lala command accuracy

Use only supported commands and flags:

- `lala security audit [--deep] [--fix] [--json]`
- `lala status` / `lala status --deep`
- `lala health --json`
- `lala update status`
- `lala cron add|list|runs|run`

Do not invent CLI flags or imply Lala enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Lala compaction:

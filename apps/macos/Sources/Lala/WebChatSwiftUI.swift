...
private let webChatSwiftLogger = Logger(subsystem: "ai.lala", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "lala.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: LalaChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> LalaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(LalaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> LalaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                LalaGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: LalaProtocol.ModelChoice) -> LalaChatModelChoice {
        LalaChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<LalaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any LalaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = LalaChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: LalaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<LalaChatView>) -> NSViewController
    {

...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        ...
        case .pairingRequired:
            "Approve this device from an already-paired Lala client. In your Lala chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .tokenRequired, .gatewayTokenNotConfigured:
            "No token yet? Generate one on the gateway host with `lala doctor --generate-gateway-token`, then set it as `gateway.auth.token`."
        case .pairingRequired:
            "If you do not have another paired Lala client yet, approve the pending request on the gateway host with `lala devices approve`."
    ...
    var statusMessage: String {
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired Lala client, run /pair approve, then check the connection again."
        }
    }
}

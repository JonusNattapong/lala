...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.lala.mac`) even if the shipped
    ...
    {
        if environment["LALA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "lala.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "lala.nixMode") { return true }

...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.lala", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> LalaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: LalaSessionPreviewEntry,
    ...
    private static func cache(payload: LalaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: LalaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [LalaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [LalaKit.AnyCodable]) -> [LalaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(LalaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: LalaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: LalaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: LalaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: LalaChatMessage) -> String? {

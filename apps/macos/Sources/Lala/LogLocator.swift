...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["LALA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/lala")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("lala-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("lala-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/lala/ (rolling or stdout), or nil if none exist.

...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: LalaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.lala", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: LalaKit.AnyCodable]()) {
                $0[$1.key] = LalaKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: LalaProtocol.AnyCodable?) -> [String: LalaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: LalaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: LalaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: LalaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("lala.control.heartbeat")
    static let controlAgentEvent = Notification.Name("lala.control.agent")
}

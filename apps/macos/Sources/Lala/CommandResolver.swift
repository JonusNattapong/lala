...
enum CommandResolver {
    private static let projectRootDefaultsKey = "lala.gatewayProjectRootPath"
    private static let helperName = "lala"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let lalaEntry = root.appendingPathComponent("lala.mjs").path
        if FileManager().isReadableFile(atPath: lalaEntry) { return lalaEntry }
        let binEntry = root.appendingPathComponent("bin/lala.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/lala")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let lalaPaths = self.lalaManagedPaths(home: home)
        if !lalaPaths.isEmpty {
            extras.insert(contentsOf: lalaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + lalaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func lalaManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".lala"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func lalaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectLalaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("lala.mjs").path,
            root.appendingPathComponent("bin/lala.js").path,
        ]
    ...
    static func hasAnyLalaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.lalaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let lalaPath = self.projectLalaExecutable(projectRoot: root) {
            return [lalaPath, subcommand] + extraArgs
        }
        if let lalaPath = self.lalaExecutable(searchPaths: searchPaths) {
            return [lalaPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "lala", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            lala entrypoint missing (looked for dist/index.js or lala.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real lala CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/lala"
        ...
        \(cliSection)
        \(projectSection)
        if command -v lala >/dev/null 2>&1; then
          CLI="$(command -v lala)"
          lala \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/lala.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/lala.mjs"
            node "$PRJ/lala.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/lala.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/lala.js"
            node "$PRJ/bin/lala.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent lala"
          pnpm --silent lala \(quotedArgs);
        else
          echo "lala CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? LalaConfigFile.loadDict()

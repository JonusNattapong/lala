...
enum LalaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = LalaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = LalaFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        LalaLogging.bootstrapIfNeeded()
        let label = LalaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct LalaOSLogHandler: AppLogLevelBackedHandler {
...
struct LalaFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = LalaLogging.parseLabel(self.label)

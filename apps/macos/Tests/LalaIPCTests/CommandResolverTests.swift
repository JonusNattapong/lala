...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open lala binary`() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let lalaPath = tmp.appendingPathComponent("node_modules/.bin/lala")
        try makeExecutableForTests(at: lalaPath)

        let cmd = CommandResolver.lalaCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([lalaPath.path, "gateway"]))
    }
    ...
    @Test func `prefers open lala binary over pnpm`() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let lalaPath = binDir.appendingPathComponent("lala")
        ...
        #expect(cmd.prefix(2).elementsEqual([lalaPath.path, "rpc"]))
    }

    @Test func `uses open lala binary without node runtime`() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "lala", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "lala", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("lala@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/lala", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "lala@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/lala'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("lala"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == lalaPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }

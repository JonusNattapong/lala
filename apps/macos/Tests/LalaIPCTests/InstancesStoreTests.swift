...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: LalaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: LalaProtocol.AnyCodable] = [
            "presence": .init([LalaProtocol.AnyCodable(entry)]),
        ]
        let payload = LalaProtocol.AnyCodable(payloadMap)

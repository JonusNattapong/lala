...
struct ChatMessageBubble: View {
    let message: LalaChatMessage
    let style: LalaChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? LalaChatTheme.userText : LalaChatTheme.assistantText
    ...
    private var inlineAttachments: [LalaChatMessageContent] {
    ...
    private var toolCalls: [LalaChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [LalaChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? LalaChatTheme.userBubble
        }
        if self.style == .onboarding {
            return LalaChatTheme.onboardingAssistantBubble
        }
        return LalaChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return LalaChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: LalaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? LalaChatTheme.userText : LalaChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: LalaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(LalaChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [LalaChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: LalaChatTheme.assistantText)
            }
        }
    }
}

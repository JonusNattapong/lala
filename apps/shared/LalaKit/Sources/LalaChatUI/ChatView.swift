...
@MainActor
public struct LalaChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: LalaChatViewModel
    ...
    private var visibleMessages: [LalaChatMessage] {
        let base: [LalaChatMessage]
    ...
    private func mergeToolResults(in messages: [LalaChatMessage]) -> [LalaChatMessage] {
    ...
    private func isToolResultMessage(_ message: LalaChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: LalaChatMessage) -> Bool {
    ...
    private func primaryText(in message: LalaChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: LalaChatMessage) -> Bool {
    ...
    private func toolCalls(in message: LalaChatMessage) -> [LalaChatMessageContent] {
    ...
    private func inlineToolResults(in message: LalaChatMessage) -> [LalaChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: LalaChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: LalaChatMessage) -> String {

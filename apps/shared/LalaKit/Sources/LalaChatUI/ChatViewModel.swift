...
private let chatUILogger = Logger(subsystem: "ai.lala", category: "LalaChatUI")

@MainActor
@Observable
public final class LalaChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [LalaChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [LalaChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [LalaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [LalaChatPendingToolCall] = []
    public private(set) var sessions: [LalaChatSessionEntry] = []
    private let transport: any LalaChatTransport
    private var sessionDefaults: LalaChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: LalaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any LalaChatTransport,
    ...
    public var sessionChoices: [LalaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [LalaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: LalaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [LalaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: LalaChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: LalaChatMessage) -> LalaChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> LalaChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return LalaChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: LalaChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [LalaChatMessage],
        incoming: [LalaChatMessage]) -> [LalaChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [LalaChatMessage]) -> [LalaChatMessage] {
        var result: [LalaChatMessage] = []
    ...
    private static func dedupeKey(for message: LalaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [LalaChatMessageContent] = [
            LalaChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> LalaChatAttachmentPayload in
            LalaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> LalaChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = LalaChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: LalaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: LalaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: LalaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = LalaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> LalaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)

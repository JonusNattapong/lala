...
enum LalaPlatformImageFactory {
    static func image(_ image: LalaPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)

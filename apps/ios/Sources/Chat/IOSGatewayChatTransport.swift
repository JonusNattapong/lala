...
struct IOSGatewayChatTransport: LalaChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.lala", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> LalaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(LalaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> LalaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(LalaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [LalaChatAttachmentPayload]) async throws -> LalaChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [LalaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(LalaChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(LalaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: LalaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: LalaChatEventPayload.self)
                        {

...
final class MotionService: MotionServicing {
    func activities(params: LalaMotionActivityParams) async throws -> LalaMotionActivityPayload {
        ...
        let mapped: [LalaMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return LalaMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: LalaPedometerParams) async throws -> LalaPedometerPayload {
        ...
        let payload: LalaPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = LalaPedometerPayload(

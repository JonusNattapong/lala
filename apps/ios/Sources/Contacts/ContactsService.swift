...
final class ContactsService: ContactsServicing {
    ...
    func search(params: LalaContactsSearchParams) async throws -> LalaContactsSearchPayload {
        ...
        return LalaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: LalaContactsAddParams) async throws -> LalaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return LalaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return LalaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> LalaContactPayload {
        LalaContactPayload(

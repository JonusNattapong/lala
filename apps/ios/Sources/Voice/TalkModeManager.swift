...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__LALA_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<LalaTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.lala", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = LalaTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> LalaTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return LalaTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> LalaTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> LalaTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> LalaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = LalaTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: LalaTalkPTTStopPayload) {

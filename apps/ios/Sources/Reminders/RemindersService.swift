...
final class RemindersService: RemindersServicing {
    func list(params: LalaRemindersListParams) async throws -> LalaRemindersListPayload {
        ...
        let payload: [LalaReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return LalaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: LalaRemindersAddParams) async throws -> LalaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = LalaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return LalaRemindersAddPayload(reminder: payload)
    }

...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> LalaDeviceStatusPayload {
        ...
        return LalaDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> LalaBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: LalaBatteryState = switch device.batteryState {
        ...
        return LalaBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> LalaThermalStatusPayload {
        let state: LalaThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return LalaThermalStatusPayload(state: state)
    }

    private func storageStatus() -> LalaStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return LalaStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }

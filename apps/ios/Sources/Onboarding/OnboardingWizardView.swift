...
struct OnboardingWizardView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Connect to your Lala gateway")
                .font(.subheadline)
                .foregroundStyle(.secondary)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `lala devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `lala devices approve` (or `lala devices approve <requestId>`)\n"
                            + "2) `/pair approve` in your Lala chat\n"
                            + "\(requestLine)\n"
                            + "Lala will also retry automatically when you return to this app.")
                }
            }
    ...
    private func initializeState() {
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "lala.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "lala.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "lala.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "lala.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:

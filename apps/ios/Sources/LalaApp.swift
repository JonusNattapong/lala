...
@MainActor
final class LalaAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.lala.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.lala.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.lala.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "lala.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "lala.watch.promptId"
    static let sessionKeyKey = "lala.watch.sessionKey"
    static let actionPrimaryIDKey = "lala.watch.action.primary.id"
    static let actionPrimaryLabelKey = "lala.watch.action.primary.label"
    static let actionSecondaryIDKey = "lala.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "lala.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "lala.watch.action.primary"
    static let actionSecondaryIdentifier = "lala.watch.action.secondary"
    static let actionIdentifierPrefix = "lala.watch.action."
    static let actionIDKeyPrefix = "lala.watch.action.id."
    static let actionLabelKeyPrefix = "lala.watch.action.label."
    static let categoryPrefix = "lala.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: LalaWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> LalaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return LalaWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Lala" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [LalaWatchAction]) -> [UNNotificationAction] {
...
struct LalaApp: App {
    ...
    @UIApplicationDelegateAdaptor(LalaAppDelegate.self) private var appDelegate
...
extension LalaApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Lala: installing uncaught exception handler")

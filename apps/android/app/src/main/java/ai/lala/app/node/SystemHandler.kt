...
private const val NOTIFICATION_CHANNEL_BASE_ID = "lala.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Lala Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Lala Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Lala Active")
      }

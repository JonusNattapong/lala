...
type DriverMode = "token" | "webhook" | "lala";
...
function resolveStateDir(): string {
  const override = process.env.LALA_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.LALA_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".lala");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver lala] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Lala spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|lala> Driver transport mode (default: token)\n" +
    ...
    "  --lala-bin <path>        Lala CLI binary for driver=lala (default: lala)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  LALA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  LALA_DISCORD_SMOKE_DRIVER\n" +
    "  LALA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  LALA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  LALA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  LALA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  LALA_DISCORD_SMOKE_AGENT\n" +
    "  LALA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  LALA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  LALA_DISCORD_SMOKE_POLL_MS\n" +
    "  LALA_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  LALA_DISCORD_SMOKE_LALA_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.LALA_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.LALA_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.LALA_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.LALA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.LALA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.LALA_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.LALA_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.LALA_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.LALA_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.LALA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.LALA_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.LALA_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const lalaBin =
    resolveArg("--lala-bin") || process.env.LALA_DISCORD_SMOKE_LALA_BIN || "lala";
...
async function lalaCliJson<T>(params: { lalaBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.lalaBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`lala ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithLala(params: {
  ...
  const response = await lalaCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    lalaBin: params.lalaBin,
    args: [
      "message",
      "read",
      "--channel",
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `lala-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("lala message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Lala.`,
        diagnostics: {
          bindingCandidates: [
            {

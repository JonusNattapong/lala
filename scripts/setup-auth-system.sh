#!/bin/bash
# Setup Lala Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'lala-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Lala message: Send warning via Lala itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/lala-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/lala-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/lala-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now lala-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/LalaAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/LalaAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Lala*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (LalaAuth) shows status and opens auth URL if needed."
echo "The full widget (LalaAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status lala-auth-monitor.timer"
echo ""
echo "Quick commands:"

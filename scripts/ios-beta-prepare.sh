...
PUSH_RELAY_BASE_URL="${LALA_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid LALA_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid LALA_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid LALA_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid LALA_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing LALA_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(LALA_URL_SLASH)$(LALA_URL_SLASH)#g'
)"
...
LALA_CODE_SIGN_STYLE = Automatic
LALA_DEVELOPMENT_TEAM = ${TEAM_ID}
LALA_IOS_SELECTED_TEAM = ${TEAM_ID}
LALA_APP_BUNDLE_ID = ai.lala.client
LALA_SHARE_BUNDLE_ID = ai.lala.client.share
LALA_ACTIVITY_WIDGET_BUNDLE_ID = ai.lala.client.activitywidget
LALA_WATCH_APP_BUNDLE_ID = ai.lala.client.watchkitapp
LALA_WATCH_EXTENSION_BUNDLE_ID = ai.lala.client.watchkitapp.extension
LALA_APP_PROFILE =
LALA_SHARE_PROFILE =
LALA_PUSH_TRANSPORT = relay
LALA_PUSH_DISTRIBUTION = official
LALA_URL_SLASH = /
LALA_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
LALA_PUSH_APNS_ENVIRONMENT = production
EOF

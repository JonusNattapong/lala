# Lala Installer for Windows (PowerShell)
# Usage: iwr -useb https://lala.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://lala.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\lala",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Install-LalaNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing Lala (lala@$Version)..." -Level info
...
function Install-LalaGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Lala from git..." -Level info
    ...
    @"
@echo off
node "%~dp0..\lala\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\lala.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'lala onboard' to complete setup" -Level info
    }

...
# Lala Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://lala.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Lala."
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦋 Lala Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://lala.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.lala.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_lala_paths() {
    ...
    rm -rf "$npm_root"/.lala-* "$npm_root"/lala 2>/dev/null || true
}

extract_lala_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_lala_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/lala" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/lala"|"/usr/local/bin/lala")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/lala/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale lala symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing lala binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Lala package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_lala_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Lala npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*lala" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_lala_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_lala_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Lala npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an lala binary already exists"
            if [[ -n "$conflict" ]]; then
...
print_usage() {
    cat <<EOF
Lala installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://lala.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/lala)
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://lala.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://lala.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://lala.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Lala checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Lala source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_lala_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"lala"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://lala.ai/install.sh | bash"
}
...
ensure_lala_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/lala" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/lala" ]]; then
        ln -sf "$npm_root/lala/dist/entry.js" "${npm_bin}/lala"
        ui_info "Created lala bin link at ${npm_bin}/lala"
    fi
    return 0
}

# Check for existing Lala installation
check_existing_lala() {
    if [[ -n "$(type -P lala 2>/dev/null || true)" ]]; then
        ui_info "Existing Lala installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make lala show as \"command not found\" in new terminals."
...
warn_lala_not_found() {
    ui_warn "Installed, but lala is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t lala 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named lala; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_lala_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P lala 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/lala" ]]; then
        echo "${npm_bin}/lala"
        return 0
    fi
...
install_lala_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/JonusNattapong/lala.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Lala from git checkout: ${repo_dir}"
    else
        ui_info "Installing Lala from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Lala" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Lala" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/lala" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/lala"
    ui_success "Lala wrapper installed to \$HOME/.local/bin/lala"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view lala dist-tags.beta 2>/dev/null || true)"
...
install_lala() {
    ...
    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "lala" ]]; then
        if ! resolve_lala_bin &> /dev/null; then
            ...
            cleanup_npm_lala_paths
            install_lala_npm "lala@next"
        fi
    fi

    ensure_lala_bin_link || true

    ui_success "Lala installed"
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run lala onboard to finish setup"
        return
    fi
...
if ! declare -F extract_lala_semver >/dev/null 2>&1; then
...
resolve_lala_version() {
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/lala/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/lala/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_lala_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Lala checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
    ...
    installed_version=$(resolve_lala_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦋 Lala installed successfully (${installed_version})!"
    else
        ui_celebrate "🦋 Lala installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/lala"
        ui_kv "Update command" "lala update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://lala.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "No TTY; run lala doctor and lala plugins update --all manually"
        fi
    else

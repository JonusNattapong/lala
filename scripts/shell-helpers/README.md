...
Inspired by Simon Willison's [Running Lala in Docker](https://til.simonwillison.net/llms/lala-docker).
...
| `clawdock-cli <command>`  | Run Lala CLI commands                      |
...
| `clawdock-cd`        | Jump to the Lala project directory    |
| `clawdock-config`    | Open the Lala config directory        |
...
```bash
lala channels login --channel whatsapp --verbose
```
...
2. Configure it in the Lala config
...
- Bash or Zsh shell
- Lala project (from `docker-setup.sh`)

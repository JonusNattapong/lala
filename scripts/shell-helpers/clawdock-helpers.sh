#!/usr/bin/env bash
# LalaDock - Docker helpers for Lala
# Inspired by Simon Willison's "Running Lala in Docker"
# https://til.simonwillison.net/llms/lala-docker
#
# Installation:
...
# Common paths to check for Lala
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦋 Found Lala at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/lala"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Lala not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/JonusNattapong/lala.git ~/lala"
    echo "  cd ~/lala && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^LALA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d lala-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart lala-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f lala-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec lala-gateway \
    bash -c 'echo "alias lala=\"./lala.mjs\"" > /tmp/.bashrc_lala && bash --rcfile /tmp/.bashrc_lala'
}
...
clawdock-cli() {
  _clawdock_compose run --rm lala-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build lala-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "LALA_GATEWAY_TOKEN=$token" lala-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" lala-gateway \
    bash -c './lala.mjs config set gateway.remote.token "$TOKEN" && ./lala.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec lala-gateway \
    bash -c "./lala.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart lala-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm lala-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec lala-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'lala config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦋 LalaDock - Docker Helpers for Lala${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (lala alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to lala project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.lala)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'lala channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'lala status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.lala.ai${_CLR_RESET}"
  echo ""
}

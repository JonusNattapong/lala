...
const EXPECTED_REPOSITORY_URL = "https://github.com/JonusNattapong/lala";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "lala") {
    errors.push(`package.json name must be "lala"; found "${pkg.name ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`lala-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `lala-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}

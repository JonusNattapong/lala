...
INSTALL_URL="${LALA_INSTALL_URL:-https://lala.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${LALA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${LALA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="lala"
PACKAGE_NAME="${LALA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (LALA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${LALA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${LALA_INSTALL_LATEST_OUT:-}"
fi

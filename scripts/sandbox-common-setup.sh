...
BASE_IMAGE="${BASE_IMAGE:-lala-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-lala-sandbox-common:bookworm-slim}"
...
LALA_DOCKER_BUILD_USE_BUILDX="${LALA_DOCKER_BUILD_USE_BUILDX:-0}"
LALA_DOCKER_BUILD_CACHE_FROM="${LALA_DOCKER_BUILD_CACHE_FROM:-}"
LALA_DOCKER_BUILD_CACHE_TO="${LALA_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${LALA_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${LALA_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${LALA_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${LALA_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${LALA_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=lala.sandbox=1)
NOTE

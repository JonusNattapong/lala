...
const shouldBuild = (deps) => {
  if (deps.env.LALA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.LALA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[lala] ${message}\n`);
};

const runLala = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["lala.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });

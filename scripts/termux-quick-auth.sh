...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/lala/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Lala Re-Auth" -c "After getting key, run: ssh $SERVER '~/lala/scripts/mobile-reauth.sh'" --id lala-auth
        ;;
    *)

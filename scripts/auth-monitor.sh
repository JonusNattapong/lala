...
#
# Suggested cron: */30 * * * * /home/admin/lala/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Lala notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., lala-alerts)
...
STATE_FILE="$HOME/.lala/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Lala if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use lala
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Lala to $NOTIFY_PHONE..."
            lala send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Lala is down. Run: ssh l36 '~/lala/scripts/mobile-reauth.sh'" "urgent"
    exit 1

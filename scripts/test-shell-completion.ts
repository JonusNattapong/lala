 ...
 * `lala update`. Use it to verify the completion installation flow
...
\`lala update\`, \`lala doctor\`, and \`lala onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Lala Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`lala completion --install\` later to enable.`));
    return;
  }

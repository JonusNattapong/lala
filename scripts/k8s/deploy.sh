...
#
# Environment:
#   OPENCLAW_NAMESPACE   Kubernetes namespace (default: lala)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANIFESTS="$SCRIPT_DIR/manifests"
NS="${OPENCLAW_NAMESPACE:-lala}"
...
_apply_secret() {
  ...
  if kubectl get secret lala-secrets -n "$NS" &>/dev/null; then
    ...
    EXISTING_TOKEN="$(kubectl get secret lala-secrets -n "$NS" -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d)"
  ...
  if $SHOW_TOKEN; then
    ...
    echo "  kubectl get secret lala-secrets -n $NS -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d && echo"
  fi
}
...
if $SHOW_TOKEN; then
  ...
  echo "  $(kubectl get secret lala-secrets -n "$NS" -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d)"
echo ""
fi

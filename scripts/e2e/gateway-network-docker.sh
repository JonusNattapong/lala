...
IMAGE_NAME="lala-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="lala-net-e2e-$$"
GW_NAME="lala-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "LALA_GATEWAY_TOKEN=$TOKEN" \
  -e "LALA_SKIP_CHANNELS=1" \
  -e "LALA_SKIP_GMAIL_WATCHER=1" \
  -e "LALA_SKIP_CRON=1" \
  -e "LALA_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \

...
export function buildConfigureCandidates(config: LalaConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: LalaConfig;
  authoredLalaConfig?: LalaConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredLalaConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const lalaCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...lalaCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "lala secrets configure",

...
export function isCommandEnabled(cfg: LalaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: LalaConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as LalaConfig),
...
export function getCommandDetection(_cfg?: LalaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: LalaConfig) {

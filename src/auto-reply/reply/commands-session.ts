...
import { scheduleGatewaySigusr1Restart, triggerLalaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Lala in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerLalaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Lala via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

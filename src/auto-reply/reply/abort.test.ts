...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "lala-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop lala!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@lala_bot", { botUsername: "lala_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@lala_bot", { botUsername: "lala_bot" })).toBe(true);

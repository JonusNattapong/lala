...
describe("resolveInitialTuiAgentId", () => {
  const cfg: LalaConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/lala" },
        { id: "ops", workspace: "/tmp/lala/projects/ops" },
      ],
    },
  };
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run lala devices list");
    expect(state.pairingHint).toContain("lala devices list");
  });

...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set LALA_GATEWAY_TOKEN / LALA_GATEWAY_PASSWORD, pass --token / --password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.LALA_GATEWAY_TOKEN ?? env.OPENCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.LALA_GATEWAY_PASSWORD ?? env.OPENCLAW_GATEWAY_PASSWORD);

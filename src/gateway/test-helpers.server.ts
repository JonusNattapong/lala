...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "LALA_STATE_DIR",
  "LALA_CONFIG_PATH",
  "LALA_SKIP_BROWSER_CONTROL_SERVER",
  "LALA_SKIP_GMAIL_WATCHER",
  "LALA_SKIP_CANVAS_HOST",
  "LALA_BUNDLED_PLUGINS_DIR",
  "LALA_SKIP_CHANNELS",
  "LALA_SKIP_PROVIDERS",
  "LALA_SKIP_CRON",
  "LALA_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "lala-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.LALA_STATE_DIR = path.join(tempHome, ".lala");
  delete process.env.LALA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.LALA_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.LALA_SKIP_GMAIL_WATCHER = "1";
  process.env.LALA_SKIP_CANVAS_HOST = "1";
  process.env.LALA_SKIP_CHANNELS = "1";
  process.env.LALA_SKIP_PROVIDERS = "1";
  process.env.LALA_SKIP_CRON = "1";
  process.env.LALA_TEST_MINIMAL_GATEWAY = "1";
  process.env.LALA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "lala-test-no-bundled-extensions")
    : "lala-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".lala-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".lala-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__lalaTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__lalaTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["LALA_GATEWAY_TOKEN"]);
  const prev = process.env.LALA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.LALA_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.LALA_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "lala-logs-"));
    const older = path.join(tempDir, "lala-2026-01-20.log");
    const newer = path.join(tempDir, "lala-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "lala-2026-01-22.log") });

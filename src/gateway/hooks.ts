...
export function resolveHooksConfig(cfg: LalaConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: LalaConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-lala-token"] === "string" ? req.headers["x-lala-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;

...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("LALA_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "lala", input: "hi" }),
      });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "lala:beta", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "lala", input: "hi" },
        { "x-lala-message-channel": "custom-client-channel" },
      );

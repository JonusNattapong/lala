...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.LALA_TAILNET_DNS;
    delete process.env.LALA_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.LALA_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.LALA_TAILNET_DNS = "studio.tailnet.ts.net.";

...
let lastCreateLalaToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/lala-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateLalaToolsContext?.agentTo,
          agentThreadId: lastCreateLalaToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createLalaTools: (ctx: Record<string, unknown>) => {
      lastCreateLalaToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateLalaToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateLalaToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-lala-message-to": "channel:24514",
        "x-lala-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });

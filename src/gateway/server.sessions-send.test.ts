...
const { createLalaTools } = await import("../agents/lala-tools.js");
...
type SessionSendTool = ReturnType<typeof createLalaTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createLalaTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["LALA_GATEWAY_PORT", "LALA_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.LALA_GATEWAY_PORT = String(gatewayPort);
  process.env.LALA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("LALA_CONFIG_PATH missing in gateway test environment");
      }

...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.LALA_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.LALA_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set LALA_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: lala doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__LALA_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;

...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("lala doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with LALA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with LALA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.LALA_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://lala-gateway.ai:18789",
      onConnectError,
    });

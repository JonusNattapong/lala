    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "lala", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-lala-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "lala", messages: [{ role: "user", content: "hi" }] },
          {
            "x-lala-agent-id": "beta",
            "x-lala-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Lala.");
      }

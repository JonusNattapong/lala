...
import { createLalaTools } from "../agents/lala-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-lala-message-channel") ?? "");
  const accountId = getHeader(req, "x-lala-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-lala-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-lala-thread-id")?.trim() || undefined;
  ...
  const allTools = createLalaTools({

...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: LalaConfig;
  nextConfig: LalaConfig;
  workspaceDir: string;
...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.lala.ai/gateway/health",
          "https://docs.lala.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.lala.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.lala/lala.json (gateway.auth.token) or LALA_GATEWAY_TOKEN (legacy: LALA_GATEWAY_TOKEN).",
        ...
        `Open the dashboard anytime: ${formatCliCommand("lala dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Lala."
            : "Copy/paste this URL in a browser on this machine to control Lala.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("lala dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    ["Back up your agent workspace.", "Docs: https://docs.lala.ai/concepts/agent-workspace"].join(
      "\n",
    ),
    "Workspace backup",
  );
  ...
  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.lala.ai/security",
    "Security",
  );
    ...
    if (webSearchEnabled !== false && hasKey) {
      await prompter.note(
        [
          ...
          `Provider: ${label}`,
          ...(keySource ? [keySource] : []),
          "Docs: https://docs.lala.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
    } else if (!hasKey) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("lala configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.lala.ai/tools/web"}`,
          "Docs: https://docs.lala.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("lala configure --section web")}`,
          "",
          "Docs: https://docs.lala.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://lala.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Lala."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Lala.",
  );

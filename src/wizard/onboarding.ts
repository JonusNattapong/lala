...
const OAUTH_WIZARD_NOTES: Partial<Record<AuthChoice, { title: string; body: string }>> = {
  ...
  chutes: {
    title: "OAuth",
    body: "Chutes uses OAuth. The wizard will guide you through opening a browser and completing the callback.",
  },
  "github-copilot": {
    title: "OAuth",
    body: "GitHub Copilot uses GitHub device login. The wizard will open the login flow next.",
  },
  "google-gemini-cli": {
    title: "OAuth",
    body: "Google Gemini CLI uses OAuth. The wizard will guide you through the browser sign-in flow next.",
  },
  "qwen-portal": {
    title: "OAuth",
...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Lala is a hobby project and still in beta. Expect sharp edges.",
      "By default, Lala is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Lala is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Lala.",
      ...
      "",
      "Must read: https://docs.lala.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Lala onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: LalaConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.lala.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("lala doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("lala configure")}.`;
  ...
  let nextConfig: LalaConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
  ...
  const oauthWizardNote = OAUTH_WIZARD_NOTES[authChoice];
  if (oauthWizardNote) {
    await prompter.note(oauthWizardNote.body, oauthWizardNote.title);
  }

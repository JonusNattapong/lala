...
`;
    ...
    expect(parsed.lala.emoji).toBe("💾");
    expect(parsed.lala.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.lala.emoji).toBe("📝");
    expect(parsed.lala.events).toEqual(["command"]);
    expect(parsed.lala.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.lala.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"lala": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"lala": {"events": ["test"]}}');
  });
...
describe("resolveLalaMetadata", () => {
  it("extracts lala metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveLalaMetadata(frontmatter);
  ...
  it("returns undefined when lala key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        lala: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Lala" },
            { id: "npm", kind: "npm", package: "@lala/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@lala/hook");
  });
...
`;
    ...
    const lala = resolveLalaMetadata(frontmatter);
    expect(lala).toBeDefined();
    expect(lala?.emoji).toBe("💾");
    expect(lala?.events).toEqual(["command:new", "command:reset"]);
    expect(lala?.requires?.config).toEqual(["workspace.dir"]);
    expect(lala?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const lala = resolveLalaMetadata(frontmatter);
    expect(lala?.emoji).toBe("disk");
    expect(lala?.events).toEqual(["command:new"]);
  });
});

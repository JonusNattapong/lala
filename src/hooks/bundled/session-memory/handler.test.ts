...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lala-session-memory-"));
});
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): LalaConfig {
  return {
    agents: { defaults: { workspace: tempDir } },

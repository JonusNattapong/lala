...
This directory contains hooks that ship with Lala. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.lala/lala.json`:
...
2. Restart gateway: `pkill -9 -f 'lala.*gateway' && pnpm lala gateway`
3. Enable the hook: `lala hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.lala.ai/automation/hooks

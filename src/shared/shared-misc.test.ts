...
describe("shared/frontmatter", () => {
  ...
  test("resolveLalaManifestBlock parses JSON5 metadata and picks lala block", () => {
    const frontmatter = {
      metadata: "{ lala: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveLalaManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

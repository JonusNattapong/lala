...
function createDefaultThreadConfig(): LoadedConfig {
  return {
    ...
    session: { store: "/tmp/lala-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\blala\\b"] },
        },
      });
      ...
      await handler(createGuildMessageEvent({ messageId: "m2", content: "lala: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "Lala" },
          },
        },
        eventPatch: {

...
function resolveVoiceTtsConfig(params: { cfg: LalaConfig; override?: TtsConfig }): {
  cfg: LalaConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredLalaTmpDir(), "discord-voice-"));

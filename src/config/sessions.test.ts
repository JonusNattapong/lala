...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lala-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ LALA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-lala",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-lala#general");
  });
  ...
  it("derives session transcripts dir from LALA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { LALA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when LALA_STATE_DIR differs from stored paths", () => {

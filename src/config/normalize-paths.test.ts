  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.lala/logs/lala.log" },
        hooks: {
          path: "~/.lala/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".lala", "logs", "lala.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".lala", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".lala", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".lala", "agents", "main"));

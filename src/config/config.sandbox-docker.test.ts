...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("lala-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "lala-sandbox-browser-global" },
      agentBrowser: { network: "lala-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("lala-sandbox-browser-agent");
  });

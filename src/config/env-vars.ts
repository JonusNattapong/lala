...
function collectConfigEnvVarsByTarget(cfg?: LalaConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: LalaConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: LalaConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: LalaConfig): Record<string, string> {
...
export function applyConfigEnvVars(cfg: LalaConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.LALA_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth

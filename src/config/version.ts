...
export function parseLalaVersion(raw: string | null | undefined): LalaVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareLalaVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseLalaVersion(a);
  const parsedB = parseLalaVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }

...
import type { LalaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: LalaConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: LalaConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: LalaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = LalaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as LalaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as LalaConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as LalaConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as LalaConfig,
  };
}

...
import { buildWebSearchProviderConfig, withTempHome, writeLalaConfig } from "./test-helpers.js";
import { LalaSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = LalaSchema.safeParse({
      $schema: "https://lala.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://lala.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = LalaSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = LalaSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeLalaConfig(home, {
        gateway: { bind: "${LALA_BIND}" },
      });

      const prev = process.env.LALA_BIND;
      process.env.LALA_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.LALA_BIND;
        } else {
          process.env.LALA_BIND = prev;
        }
      }
    });
  });

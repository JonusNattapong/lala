...
import type { ConfigFileSnapshot } from "./types.lala.js";
import { LalaSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(LalaSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.lala/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/lala/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/lala/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/lala/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/lala/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = LalaSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "LalaConfig";
    const hints = mainSchemaHints;

...
import type { LalaConfig } from "./config.js";
...
function isWhatsAppConfigured(cfg: LalaConfig): boolean {
...
function isGenericChannelConfigured(cfg: LalaConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: LalaConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: LalaConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: LalaConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: LalaConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: LalaConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: LalaConfig, pluginId: string): LalaConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;

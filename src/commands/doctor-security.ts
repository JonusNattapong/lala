...
import type { LalaConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: LalaConfig): string[] {
...
export async function noteSecurityWarnings(cfg: LalaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("lala security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.lala/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("lala approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.lala.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("lala configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("lala config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("lala doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("lala config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('lala config set session.dmScope "per-channel-peer"') +

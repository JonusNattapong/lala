...
import { readConfigFileSnapshot, writeConfigFile, type LalaConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("lala config set gateway.auth.mode token")} or ${formatCliCommand("lala config set gateway.auth.mode password")}.`,
  ].join(" ");
}

...
import { LalaSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: LalaConfig): {
  config: LalaConfig;
  removed: string[];
} {
  const parsed = LalaSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: LalaConfig): void {

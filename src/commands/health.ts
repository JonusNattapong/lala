...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.LALA_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: LalaConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.LALA_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {

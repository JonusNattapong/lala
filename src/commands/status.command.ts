...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByLala ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByLala ? "installed · " : "";
  ...
  runtime.log(theme.heading("Lala status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`lala devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("lala devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("lala devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("lala security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("lala security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.lala.ai/faq");
  runtime.log("Troubleshooting: https://docs.lala.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("lala status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("lala logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("lala status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("lala gateway probe")}`);
  }
}

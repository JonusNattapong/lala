...
export function normalizeCompatibilityConfigValues(cfg: LalaConfig): {
  config: LalaConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: LalaConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as LalaConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as LalaConfig["browser"],
    };
    changes.push(

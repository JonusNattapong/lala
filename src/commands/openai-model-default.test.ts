...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: LalaConfig },
  primary: string,
) {
...
function expectConfigUnchanged(applied: { changed: boolean; next: LalaConfig }, cfg: LalaConfig) {
  expect(applied.changed).toBe(false);
  expect(applied.next).toEqual(cfg);
}

type SharedDefaultModelCase = {
  apply: (cfg: LalaConfig) => { changed: boolean; next: LalaConfig };
  defaultModel: string;
  overrideConfig: LalaConfig;
  alreadyDefaultConfig: LalaConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: LalaConfig) => config,
      applyDefaultConfig: (config: LalaConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: LalaConfig = { agents: { defaults: {} } };

...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "lala-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.LALA_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when LALA_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.LALA_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote?.[0])).toContain("lala config set gateway.auth.mode token");
    expect(String(gatewayAuthNote?.[0])).toContain("lala config set gateway.auth.mode password");
  });
});

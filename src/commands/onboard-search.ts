...
function rawKeyValue(config: LalaConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: LalaConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: LalaConfig, provider: SearchProvider): LalaConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: LalaConfig, result: LalaConfig): LalaConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with lala configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Lala will store a reference instead of the API key.",

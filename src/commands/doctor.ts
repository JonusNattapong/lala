...
import { resolveLalaPackageRoot } from "../infra/lala-root.js";
...
function resolveMode(cfg: LalaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveLalaPackageRoot({
  ...
  let cfg: LalaConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("lala configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("lala config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("lala setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("lala config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("lala config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("lala doctor --fix")}" to apply changes.`);
  }

...
function resolveAgentName(cfg: LalaConfig, agentId: string) {
...
function resolveAgentModel(cfg: LalaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: LalaConfig): AgentSummary[] {

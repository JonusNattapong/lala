...
type PromptDefaultModelResult = { model?: string; config?: LalaConfig };
...
function resolveConfiguredModelRaw(cfg: LalaConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: LalaConfig): string[] {
...
export function applyPrimaryModel(cfg: LalaConfig, model: string): LalaConfig {
...
export function applyModelAllowlist(cfg: LalaConfig, models: string[]): LalaConfig {
  const defaults = cfg.agents?.defaults;

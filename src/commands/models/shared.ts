...
export async function loadValidConfigOrThrow(): Promise<LalaConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: LalaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: LalaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("lala agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

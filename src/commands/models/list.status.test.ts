...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveLalaAgentDir: vi.fn().mockReturnValue("/tmp/lala-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/lala-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/lala-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/lala-dev/lala.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveLalaAgentDir: mocks.resolveLalaAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/lala-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveLalaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/lala-dev/lala.json");
    expect(payload.auth.storePath).toBe("/tmp/lala-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/lala-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/lala-agent-custom");

...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveLalaAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("lala models auth setup-token")}\` or \`${formatCliCommand("lala configure")}\`.`
          : `Run \`${formatCliCommand("lala configure")}\` or set an API key env var.`;

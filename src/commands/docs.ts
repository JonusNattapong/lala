...
const SEARCH_TOOL = "https://docs.lala.ai/mcp.SearchLala";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.lala.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('lala docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.lala.ai/");
      runtime.log(`Search: ${formatCliCommand('lala docs "your query"')}`);
    }
    return;
  }

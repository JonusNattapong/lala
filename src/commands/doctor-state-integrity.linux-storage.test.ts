...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.lala", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.lala", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/lala-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/lala/.lala",
    });

    expect(result).toEqual({
      path: "/mnt/slow/lala/.lala",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".lala"), {
      platform: "darwin",
      mountInfo,
    });

...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.LALA_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.LALA_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
async function runRepair(cfg: LalaConfig) {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      LALA_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service LALA_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: LalaConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.LALA_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses LALA_GATEWAY_TOKEN when config token is missing", async () => {
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/lala/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/lala@2026.3.12/node_modules/lala/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    fsMocks.realpath.mockImplementation(async (value: string) => {
      if (value.includes("/global/5/node_modules/lala/")) {
        return value.replace(
          "/global/5/node_modules/lala/",
          "/global/5/node_modules/.pnpm/lala@2026.3.12/node_modules/lala/",
        );
      }
      return value;
    });
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: ["/usr/bin/node", "/opt/lala/dist/index.js", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/lala/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Lala gateway next.",
    );
  });
});

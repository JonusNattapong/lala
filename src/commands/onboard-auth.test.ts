...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under LALA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("lala-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lala-oauth-sync-"));
    ...
    process.env.LALA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lala-oauth-nosync-"));
    process.env.LALA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside LALA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lala-oauth-external-"));
    process.env.LALA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* LALA_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to LALA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("lala-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);

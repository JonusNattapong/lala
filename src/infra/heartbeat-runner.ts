...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: LalaConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: LalaConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: LalaConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: LalaConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: LalaConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: LalaConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: LalaConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: LalaConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();

...
export const POSIX_LALA_TMP_DIR = "/tmp/lala";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredLalaTmpDirOptions = {
...
export function resolvePreferredLalaTmpDir(
  options: ResolvePreferredLalaTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "lala" : `lala-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[lala] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Lala temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Lala temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_LALA_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_LALA_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_LALA_TMP_DIR)) {
      return POSIX_LALA_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_LALA_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_LALA_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_LALA_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_LALA_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_LALA_TMP_DIR;

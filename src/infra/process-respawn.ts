...
import { triggerLalaRestart } from "./restart.js";
 ...
 * - LALA_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.LALA_NO_RESPAWN ?? process.env.OPENCLAW_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerLalaRestart();
      if (!restart.ok) {
        return {
          mode: "failed",

...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = (process.env.LALA_TEST_TAILSCALE_BINARY ?? process.env.OPENCLAW_TEST_TAILSCALE_BINARY)?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Lala. You can keep running the web gateway without it: \`${formatCliCommand("lala gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();

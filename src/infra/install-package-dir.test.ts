...
describe("installPackageDir", () => {
  ...
  it("keeps the existing install in place when staged validation fails", async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lala-install-package-dir-"));
    ...
    await expect(listMatchingDirs(installBaseDir, ".lala-install-stage-")).resolves.toHaveLength(0);
    await expect(listMatchingDirs(installBaseDir, ".lala-install-backups")).resolves.toHaveLength(
      0,
    );
  });
  ...
  it("restores the original install if publish rename fails", async () => {
    ...
    await expect(fs.readFile(path.join(targetDir, "marker.txt"), "utf8")).resolves.toBe("old");
    await expect(listMatchingDirs(installBaseDir, ".lala-install-stage-")).resolves.toHaveLength(0);
    const backupRoot = path.join(installBaseDir, ".lala-install-backups");
  ...
  it("warns and leaves the backup in place when the install base changes before backup cleanup", async () => {
    ...
    const backupRoot = path.join(preservedInstallRoot, ".lala-install-backups");

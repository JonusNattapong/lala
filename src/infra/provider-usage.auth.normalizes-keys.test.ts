...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lala-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(base, ".lala");
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".lala", "agents", "main", "agent");

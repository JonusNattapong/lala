...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred Lala tmp root", async () => {
    const tmpRoot = resolvePreferredLalaTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideLala = path.join(os.tmpdir(), "outside-lala", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",

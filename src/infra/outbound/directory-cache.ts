...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: LalaConfig | null = null;
  ...
  get(key: string, cfg: LalaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: LalaConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: LalaConfig): void {

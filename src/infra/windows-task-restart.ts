...
function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = (env.LALA_WINDOWS_TASK_NAME ?? env.LALA_WINDOWS_TASK_NAME)?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.LALA_PROFILE ?? env.LALA_PROFILE);
}
...
export function relaunchGatewayScheduledTask(env: NodeJS.ProcessEnv = process.env): RestartAttempt {
  ...
  const scriptPath = path.join(
    resolvePreferredLalaTmpDir(),
    `lala-schtasks-restart-${randomUUID()}.cmd`,
  );

...
export function triggerLalaRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.LALA_SYSTEMD_UNIT ?? process.env.LALA_SYSTEMD_UNIT,
      process.env.LALA_PROFILE ?? process.env.LALA_PROFILE,
    );
  ...
  const label =
    ...
    process.env.LALA_LAUNCHD_LABEL ??
    resolveGatewayLaunchAgentLabel(process.env.LALA_PROFILE ?? process.env.LALA_PROFILE);

 ...
 * returning control to the caller (typically `triggerLalaRestart` →
...
/**
 * Parse lala gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("lala")) {
 ...
 * Returns only PIDs that belong to lala gateway processes (not the current process).
 */

...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__lala_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "lala-root");
...
describe("resolveLalaPackageRoot", () => {
  let resolveLalaPackageRoot: typeof import("./lala-root.js").resolveLalaPackageRoot;
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "lala");
    const pkgRoot = path.join(project, "node_modules", "lala");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "lala" }));

    expect(resolveLalaPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "lala");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "lala.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "lala" }));

    expect(resolveLalaPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveLalaPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("returns null for non-lala package roots", async () => {
    const pkgRoot = fx("not-lala");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-lala" }));

    expect(resolveLalaPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveLalaPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveLalaPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});

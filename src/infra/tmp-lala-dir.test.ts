...
import { POSIX_LALA_TMP_DIR, resolvePreferredLalaTmpDir } from "./tmp-lala-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredLalaTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `lala-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredLalaTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_LALA_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredLalaTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredLalaTmpDir", () => {
  it("prefers /tmp/lala when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_LALA_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/lala when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_LALA_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/lala when /tmp/lala is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/lala when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/lala is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/lala is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/lala is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Lala temp dir/);
  });

...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.LALA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*lala-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "lala";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "lala");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "lala-gw");
  ...
  records.push(`_lala-gw._tcp IN PTR ${instanceLabel}._lala-gw._tcp`);
  records.push(`${instanceLabel}._lala-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._lala-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; lala-content-hash: ${contentHash}\n${contentBody}`;
}

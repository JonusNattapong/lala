...
function candidateBinDirs(opts: EnsureLalaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `lala` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "lala");
...
/**
 * Best-effort PATH bootstrap so skills that require the `lala` CLI can run

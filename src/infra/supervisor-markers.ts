const SUPERVISOR_HINTS = {
  ...
  schtasks: ["LALA_WINDOWS_TASK_NAME", "LALA_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = (env.LALA_SERVICE_MARKER ?? env.LALA_SERVICE_MARKER)?.trim();
    const serviceKind = (env.LALA_SERVICE_KIND ?? env.LALA_SERVICE_KIND)?.trim();

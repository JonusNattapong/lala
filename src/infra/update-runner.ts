...
const DEFAULT_PACKAGE_NAME = "lala";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["lala", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lala-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "lala.mjs");
    ...
    const doctorStep = await runStep(
      step("lala doctor", doctorArgv, gitRoot, { LALA_UPDATE_IN_PROGRESS: "1", OPENCLAW_UPDATE_IN_PROGRESS: "1" }),
    );

...
describe("logger helpers", () => {
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `lala-${today}.log`);

    // create an old file to be pruned
    const oldPath = path.join(DEFAULT_LOG_DIR, "lala-2000-01-01.log");
...
function pathForTest() {
  const file = path.join(os.tmpdir(), `lala-log-${crypto.randomUUID()}.log`);

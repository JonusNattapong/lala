...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `lala-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "lala.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "lala.plugin.json");
  const linkedManifest = path.join(rootDir, "lala.plugin.json");

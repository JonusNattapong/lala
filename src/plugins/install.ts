...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing lala.extensions; update the plugin package to include lala.extensions (for example ["./dist/index.js"]). See https://docs.lala.ai/help/troubleshooting#plugin-install-fails-with-missing-lala-extensions';
...
function ensureLalaExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureLalaExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from lala.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-lala"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "lala security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "lala security audit --deep" after install.`,
    );
  }

...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lala-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "lala.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.lala/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without lala.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@lala/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
  ...
  it("rejects legacy plugin package shape when lala.extensions is missing", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@lala/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.lala).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses lala.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-lala"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@lala/voice-call@0.0.1",
              name: "@lala/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@lala/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@lala/voice-call@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@lala/voice-call@0.0.2-beta.1",
      name: "@lala/voice-call",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@lala/voice-call@beta"');
    }
  });
  ...
  it("allows explicit prerelease npm tags", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@lala/voice-call@0.0.2-beta.1");
    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@lala/voice-call@beta",
    });
    expect(packTmpDir).not.toBe("");
  });
});

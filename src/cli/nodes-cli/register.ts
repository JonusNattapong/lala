    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lala nodes status", "List known nodes with live status."],
          ["lala nodes pairing pending", "Show pending node pairing requests."],
          ['lala nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["lala nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.lala.ai/cli/nodes")}\n`,
    );

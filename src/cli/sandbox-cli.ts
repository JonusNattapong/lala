...
const SANDBOX_EXAMPLES = {
  main: [
    ["lala sandbox list", "List all sandbox containers."],
    ["lala sandbox list --browser", "List only browser containers."],
    ["lala sandbox recreate --all", "Recreate all containers."],
    ["lala sandbox recreate --session main", "Recreate a specific session."],
    ["lala sandbox recreate --agent mybot", "Recreate agent containers."],
    ["lala sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["lala sandbox list", "List all sandbox containers."],
    ["lala sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["lala sandbox recreate --all", "Recreate all containers."],
    ["lala sandbox recreate --session main", "Recreate a specific session."],
    ["lala sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["lala sandbox recreate --browser --all", "Recreate only browser containers."],
    ["lala sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["lala sandbox explain", "Show effective sandbox config."],
    ["lala sandbox explain --session agent:main:main", "Explain a specific session."],
    ["lala sandbox explain --agent work", "Explain an agent sandbox."],
    ["lala sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.lala.ai/cli/sandbox")}\n`,
    )
    .action(() => {

...
const STEP_LABELS: Record<string, string> = {
  ...
  "lala doctor entry": "Checking doctor entrypoint",
  "lala doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g lala@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g lala@latest --omit=optional");
  }

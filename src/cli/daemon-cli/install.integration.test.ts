...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("lala-daemon-install-int-");
    configPath = path.join(tempHome, "lala.json");
    process.env.HOME = tempHome;
    process.env.LALA_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.LALA_GATEWAY_TOKEN).toBeUndefined();
  });
});

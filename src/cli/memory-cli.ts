    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lala memory status", "Show index and provider status."],
          ["lala memory status --deep", "Probe embedding provider readiness."],
          ["lala memory index --force", "Force a full reindex."],
          ['lala memory search "meeting notes"', "Quick search using positional query."],
          [
            'lala memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["lala memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.lala.ai/cli/memory")}\n`,
    );

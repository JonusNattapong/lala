...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@lala/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@lala/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@lala/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@lala/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @lala/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@lala/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@lala/plugin-alpha@1.2.3",
        installPath: "/tmp/lala/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@lala/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @lala/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@lala/plugin-alpha@latest",
      true,
      "/tmp/lala/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),

...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Lala and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["lala update", "Update a source checkout (git)"],
        ["lala update --channel beta", "Switch to beta channel (git + npm)"],
        ["lala update --channel dev", "Switch to dev channel (git + npm)"],
        ["lala update --tag beta", "One-off update to a dist-tag or version"],
        ["lala update --dry-run", "Preview actions without changing anything"],
        ["lala update --no-restart", "Update without restarting the service"],
        ["lala update --json", "Output result as JSON"],
        ["lala update --yes", "Non-interactive (accept downgrade prompts)"],
        ["lala update wizard", "Interactive update wizard"],
        ["lala --update", "Shorthand for lala update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run lala update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.lala.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lala update status", "Show channel + version status."],
          ["lala update status --json", "JSON output."],
          ["lala update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.lala.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {

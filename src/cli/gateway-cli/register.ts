      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["lala gateway run", "Run the gateway in the foreground."],
            ["lala gateway status", "Show service status and probe reachability."],
            ["lala gateway discover", "Find local and wide-area gateway beacons."],
            ["lala gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.lala.ai/cli/gateway")}\n`,
      ),
  );

...
beforeEach(() => {
  ...
  originalHideBanner = process.env.LALA_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.LALA_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.LALA_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("lala");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "lala", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("lala-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "lala", "message", "send"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    ...
    vi.clearAllMocks();
    process.env.LALA_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "lala", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "lala", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "lala", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "lala", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "lala", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "lala", "backup", "create", "--json"],
    });

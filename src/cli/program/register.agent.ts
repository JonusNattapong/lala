...
${formatHelpExamples([
  ['lala agent --to +15555550123 --message "status update"', "Start a new session."],
  ['lala agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'lala agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'lala agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['lala agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'lala agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.lala.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.lala.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "lala agents set-identity --identity-file ~/.lala/workspace/IDENTITY.md --agent main",

...
const EXAMPLES = [
  ...
  ["lala gateway --port 18789", "Run the WebSocket Gateway locally."],
  ["lala --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["lala gateway --force", "Kill anything bound to the default gateway port, then start it."],
  ["lala gateway ...", "Gateway control via WebSocket."],
  [
    'lala agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'lala message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.lala-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.lala.ai/cli");

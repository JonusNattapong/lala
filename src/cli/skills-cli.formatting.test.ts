...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "lala-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "lala-bundled-skills-test-"));
    process.env.LALA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

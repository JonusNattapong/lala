...
function listAgents(cfg: LalaConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: LalaConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<LalaConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: LalaConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: LalaConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: LalaConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<LalaConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  LalaConfig,
  {
    bindingsRef: LalaConfig["bindings"];
    agentsRef: LalaConfig["agents"];
    sessionRef: LalaConfig["session"];
...
function resolveRouteCacheForConfig(cfg: LalaConfig): Map<string, ResolvedAgentRoute> {

...
const TEMP_MEDIA_PREFIX = "lala-media-";
...
function createGroqAudioConfig(): LalaConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): LalaConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): LalaConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: LalaConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredLalaTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        LALA_AGENT_DIR: isolatedAgentDir,

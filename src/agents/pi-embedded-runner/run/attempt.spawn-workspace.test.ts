...
vi.mock("../../docs-path.js", () => ({
  resolveLalaDocsPath: async () => undefined,
}));
...
describe("runEmbeddedAttempt sessions_spawn workspace inheritance", () => {
  ...
  it("passes the real workspace to sessions_spawn when workspaceAccess is ro", async () => {
    const realWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "lala-real-workspace-"));
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "lala-sandbox-workspace-"));
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "lala-agent-dir-"));

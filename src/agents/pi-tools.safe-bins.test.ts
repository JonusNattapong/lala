...
const bundledPluginsDirSnapshot = captureEnv(["LALA_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.LALA_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
const { createLalaCodingTools } = await import("./pi-tools.js");
...
describe("createLalaCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lala-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lala-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lala-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lala-safe-bins-redirect-",
        safeBins: ["head"],

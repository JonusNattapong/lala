...
import { markLalaExecEnv } from "../../infra/lala-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "lala.sandbox=1");
  args.push("--label", `lala.sessionKey=${params.scopeKey}`);
  args.push("--label", `lala.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `lala.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markLalaExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "lala.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`lala sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`lala sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("lala sandbox recreate --all");
}

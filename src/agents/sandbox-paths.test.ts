...
import { resolvePreferredLalaTmpDir } from "../infra/tmp-lala-dir.js";
...
async function withOutsideHardlinkInLalaTmp<T>(
  params: {
    laLaTmpDir: string;
  ...
  const hardlinkPath = path.join(params.laLaTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.laLaTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.laLaTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.laLaTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const laLaTmpDir = resolvePreferredLalaTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Lala tmp root",
      media: path.join(laLaTmpDir, "image.png"),
      expected: path.join(laLaTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Lala tmp root",
      media: pathToFileURL(path.join(laLaTmpDir, "photo.png")).href,
      expected: path.join(laLaTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Lala tmp root",
      media: path.join(laLaTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(laLaTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(laLaTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside lala tmp root",
      media: path.join(os.tmpdir(), "outside-lala", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Lala tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(laLaTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(laLaTmpDir, { recursive: true });
      const symlinkPath = path.join(laLaTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Lala tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInLalaTmp(
      {
        laLaTmpDir,
  ...
  it("rejects symlinked Lala tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInLalaTmp(

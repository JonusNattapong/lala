...
const GOOGLE_LIVE_SINGLE_KEY = "LALA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "LALA_LIVE_ANTHROPIC_KEY",
    listVar: "LALA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "LALA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `LALA_LIVE_${base}_KEY`;

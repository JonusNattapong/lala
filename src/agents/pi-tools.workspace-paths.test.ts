...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("lala-ws-", async (workspaceDir) => {
      await withTempDir("lala-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createLalaCodingTools({ workspaceDir });
          ...
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello lala");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("lala-ws-", async (workspaceDir) => {
      await withTempDir("lala-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("lala-ws-", async (workspaceDir) => {
      const cfg: LalaConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createLalaCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-lala.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("lala-sandbox-", async (sandboxDir) => {
      await withTempDir("lala-workspace-", async (workspaceDir) => {
        ...
        const tools = createLalaCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);

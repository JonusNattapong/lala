...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["LALA_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lala-subagent-"));
  ...
  it("uses isolated temp state when LALA_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "lala-test-state"));
  });
});

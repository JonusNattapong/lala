...
const defaultTools = createLalaCodingTools({ senderIsOwner: true });
...
describe("createLalaCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: LalaConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createLalaCodingTools({
    ...
    const anthropicTools = createLalaCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: LalaConfig = {
      tools: {
        exec: {
    ...
    const allowed = createLalaCodingTools({
    ...
    const denied = createLalaCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createLalaCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });

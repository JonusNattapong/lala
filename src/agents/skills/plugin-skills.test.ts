    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "lala.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "lala.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "lala.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("lala-");
  const acpxRoot = await tempDirs.make("lala-acpx-plugin-");
  const helperRoot = await tempDirs.make("lala-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("lala-");
  const pluginRoot = await tempDirs.make("lala-plugin-");
  const outsideDir = await tempDirs.make("lala-outside-");

...
function findTool(name: string, config: LalaConfig) {
  const allTools = createLalaTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: LalaConfig; env?: NodeJS.ProcessEnv }) {
...
describe("lala tools runtime web metadata wiring", () => {

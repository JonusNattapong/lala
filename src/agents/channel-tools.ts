...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as LalaConfig);
...
export function listChannelAgentTools(params: { cfg?: LalaConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: LalaConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {

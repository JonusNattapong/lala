...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    lala: process.env.LALA_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.lala === undefined) {
      delete process.env.LALA_GATEWAY_TOKEN;
    } else {
      process.env.LALA_GATEWAY_TOKEN = envSnapshot.lala;
    }
  ...
  it("uses LALA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.LALA_GATEWAY_TOKEN = "local-env-token";

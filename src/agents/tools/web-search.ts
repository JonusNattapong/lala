...
type WebSearchConfig = NonNullable<LalaConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: LalaConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "brave") {
    return {
      ...
      message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("lala configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
  ...
  return withTrustedWebSearchEndpoint(
    {
      ...
      init: {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://lala.ai",
          "X-Title": "Lala Web Search",
        },

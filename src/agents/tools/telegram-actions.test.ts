...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): LalaConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as LalaConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): LalaConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as LalaConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: LalaConfig, accountId = "media") {
    await handleTelegramAction(

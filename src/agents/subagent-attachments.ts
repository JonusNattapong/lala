...
function resolveAttachmentLimits(config: LalaConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".lala", "attachments");
  const relDir = path.posix.join(".lala", "attachments", attachmentId);

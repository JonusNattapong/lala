...
import { createLalaCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "lala-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: LalaConfig) {
    return createLalaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<LalaConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<LalaConfig["agents"]>["list"]>[number]["tools"];
  }): LalaConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createLalaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createLalaCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createLalaCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createLalaCodingTools({
      config: cfg,
    ...
    const bobTools = createLalaCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createLalaCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createLalaCodingTools({
      config: cfg,

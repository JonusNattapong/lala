...
import { planLalaModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: LalaConfig): {
  config: LalaConfig;
  sourceConfigForSecrets: LalaConfig;
} {
...
export async function ensureLalaModelsJson(
  config?: LalaConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveLalaAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planLalaModelsJson({
      cfg,

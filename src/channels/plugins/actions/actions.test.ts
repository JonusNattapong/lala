...
function telegramCfg(): LalaConfig {
  return { channels: { telegram: { botToken: "tok" } } } as LalaConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: LalaConfig; accountId?: string },
) {
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as LalaConfig;
...
function createSignalAccountOverrideCfg(): LalaConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as LalaConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as LalaConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as LalaConfig,
        accountId: undefined,
        params: {

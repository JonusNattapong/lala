...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: LalaConfig, policy: ChannelAccessPolicy) => LalaConfig;
  resolveAllowlist: (params: { cfg: LalaConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: LalaConfig; resolved: TResolved }) => LalaConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: LalaConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: LalaConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: LalaConfig, policy: ChannelAccessPolicy): LalaConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: LalaConfig, policy: ChannelAccessPolicy): LalaConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: LalaConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: LalaConfig; resolved: string[] }) => {
      calls.push("apply");

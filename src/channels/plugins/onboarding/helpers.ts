...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: LalaConfig) => Array<string | number> | undefined;
}): LalaConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: LalaConfig) => LalaConfig | Promise<LalaConfig>;
  applySet?: (
    cfg: LalaConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => LalaConfig | Promise<LalaConfig>;
}): Promise<{
  cfg: LalaConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Lala config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "LALA_SECRET",
      envVarFormatError:
        'Use an env var name like "LALA_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:

...
export function resolveTtsConfig(cfg: LalaConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = (process.env.LALA_TTS_PREFS ?? process.env.LALA_TTS_PREFS)?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: LalaConfig): string | undefined {
  ...
  for (const provider of providers) {
    ...
    try {
      if (provider === "edge") {
        ...
        const tempRoot = resolvePreferredLalaTmpDir();

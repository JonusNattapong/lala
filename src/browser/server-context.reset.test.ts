...
function localLalaProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "lala",
...
function createLocalLalaResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localLalaProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveLalaUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lala-reset-"));
    const profileDir = path.join(tempRoot, "lala");
    ...
    const ops = createLocalLalaResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveLalaUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lala-reset-no-own-"));
    const profileDir = path.join(tempRoot, "lala");
    fs.mkdirSync(profileDir, { recursive: true });

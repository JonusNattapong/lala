...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredLalaTmpDir: vi.fn(() => "/tmp/lala"),
}));
vi.mock("../infra/tmp-lala-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredLalaTmpDir.mockReturnValue("/tmp/lala");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lala-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".lala-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredLalaTmpDir.mockReturnValue("/tmp/lala-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "lala-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "lala-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredLalaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "lala-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "lala-preferred", "downloads")}${path.sep}`),
    );
  });

...
export function resolveLalaUserDataDir(profileName = DEFAULT_LALA_BROWSER_PROFILE_NAME) {
...
export async function launchLalaChrome(
  ...
  const userDataDir = resolveLalaUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_LALA_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateLalaProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦋 lala browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`lala browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`lala browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦋 lala browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );

...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedLalaRelay({
    baseUrl,
    relayAuthHeader: "x-lala-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.LALA_GATEWAY_TOKEN;
  ...
  it("accepts authenticated lala relay probe responses", async () => {

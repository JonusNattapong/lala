...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("lala");
    expect(profile?.driver).toBe("lala");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const lala = resolveProfile(resolved, "lala");
    expect(lala?.driver).toBe("lala");
    expect(lala?.cdpPort).toBe(18800);
    expect(lala?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from LALA_GATEWAY_PORT when unset", () => {
    withEnv({ LALA_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const lala = resolveProfile(resolved, "lala");
      expect(lala?.cdpPort).toBe(19012);
      expect(lala?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ LALA_GATEWAY_PORT: undefined }, () => {
      ...
      const lala = resolveProfile(resolved, "lala");
      expect(lala?.cdpPort).toBe(19022);
      expect(lala?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(lala?.cdpPort).toBe(19000);
    expect(lala?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "lala");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        lala: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("lala");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to lala profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps lala default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("lala");
    ...
    it("keeps lala default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("lala");
    ...
    it("keeps lala default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });

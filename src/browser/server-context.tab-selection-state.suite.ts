...
function seedRunningProfileState(state: ReturnType<typeof makeState>, profileName = "lala"): void {
...
async function openManagedTabWithRunningProfile(params: {
  ...
  const state = makeState("lala");
  seedRunningProfileState(state);
  const ctx = createBrowserRouteContext({ getState: () => state });
  const lala = ctx.forProfile("lala");
  return await lala.openTab(params.url ?? "http://127.0.0.1:3009");
}
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const opened = await lala.openTab("http://127.0.0.1:8080");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("lala")?.lastTargetId).toBe("CREATED");
  ...
  it("does not fail tab open when managed-tab cleanup list fails", async () => {
    ...
    const opened = await lala.openTab("http://127.0.0.1:3009");
    expect(opened.targetId).toBe("NEW");
  });
  ...
  it("blocks unsupported non-network URLs before any HTTP tab-open fallback", async () => {
    ...
    await expect(lala.openTab("file:///etc/passwd")).rejects.toBeInstanceOf(

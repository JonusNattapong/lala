...
describe("cdp.helpers", () => {
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    process.env.LALA_GATEWAY_TOKEN = "test-gateway-token";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.LALA_GATEWAY_TOKEN = prev;
      }
    }
  });
});
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    expect(listKnownProfileNames(state).toSorted()).toEqual(["chrome", "lala", "stale-removed"]);
  });
});

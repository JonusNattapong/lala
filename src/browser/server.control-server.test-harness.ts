...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.LALA_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/lala" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "lala",
        profiles: {
          lala: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchLalaChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.LALA_GATEWAY_PORT;
  process.env.LALA_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.LALA_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.LALA_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.LALA_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.LALA_GATEWAY_PASSWORD;
  } else {
    process.env.LALA_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}

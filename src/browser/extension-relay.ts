...
const RELAY_AUTH_HEADER = "x-lala-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Lala-Extension-Relay",
            revision: "0",
            userAgent: "Lala-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedLalaRelay({

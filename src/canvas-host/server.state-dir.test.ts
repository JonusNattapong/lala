...
describe("canvas host state dir defaults", () => {
  it("uses LALA_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("lala-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Lala Canvas");
      } finally {
        await handler.close();
      }
    });

...
This document describes how the Lala ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`lala acp` exposes an ACP agent over stdio and forwards prompts to a running
Lala Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Lala Gateway
...
- ACP client spawns `lala acp` and speaks ACP messages over stdio.
...
`lala acp` resolves the Gateway URL and auth from CLI flags or config:

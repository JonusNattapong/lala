...
read_when:
  - You want to place an outbound voice call from Lala
...
Voice calls for Lala via a plugin. Supports outbound notifications and
...
- Use `lala voicecall ...` or the `voice_call` tool
...
```bash
lala plugins install ./extensions/voice-call
...
```bash
lala voicecall call --to "+15555550123" --message "Hello from Lala"
lala voicecall continue --call-id <id> --message "Any questions?"
lala voicecall speak --call-id <id> --message "One moment"

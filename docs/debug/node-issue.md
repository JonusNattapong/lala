...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Lala
...
Running Lala via Node with `tsx` fails at startup with:

```
[lala] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `lala status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch lala.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node lala.mjs status` works on Node 25.

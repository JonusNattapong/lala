---
summary: "Use OpenAI via API keys or Codex subscription in Lala"
read_when:
  - You want to use OpenAI models in Lala
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Lala.
...
```bash
lala onboard --auth-choice openai-api-key
# or non-interactive
lala onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Lala forwards both through the `openai/*` Responses path.
...
lala onboard --auth-choice openai-codex

# Or run OAuth directly
lala models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Lala
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

Lala uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Lala also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Lala enables it by default for
...
Lala, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), Lala now auto-enables OpenAI server-side

---
summary: "Use MiniMax M2.5 in Lala"
read_when:
  - You want MiniMax models in Lala
...
```bash
lala plugins enable minimax-portal-auth  # skip if already loaded.
lala gateway restart  # restart if gateway is already running
lala onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/JonusNattapong/lala/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `lala.json`:
...
- Use `lala models list` and `lala models set minimax/MiniMax-M2.5` to switch.
...
- Running `lala configure` and selecting **MiniMax M2.5**, or

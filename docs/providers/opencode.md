---
summary: "Use OpenCode Zen and Go catalogs with Lala"
read_when:
...
OpenCode exposes two hosted catalogs in Lala:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Lala keeps the runtime provider ids
...
lala onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
lala onboard --auth-choice opencode-go
lala onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```

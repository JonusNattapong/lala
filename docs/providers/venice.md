---
summary: "Use Venice AI privacy-focused models in Lala"
read_when:
  - You want privacy-focused inference in Lala
...
```bash
lala onboard --auth-choice venice-api-key
```
...
```bash
lala agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Lala shows all available Venice models. Pick based on your needs:
...
```bash
lala models set venice/kimi-k2-5
lala models set venice/claude-opus-4-6
```

List all available models:

```bash
lala models list | grep venice
```

## Configure via `lala configure`

1. Run `lala configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Lala automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
lala agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
lala agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
lala agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
lala agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
lala agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `lala models list` to see currently available models. Some models may be temporarily offline.

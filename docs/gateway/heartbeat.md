...
- During heartbeat runs, Lala treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `lala sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Lala skips the heartbeat run entirely (no model call).
...
```bash
lala system event --text "Check for urgent follow-ups" --mode now
```

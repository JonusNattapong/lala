...
Lala supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `LALA_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Lala runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Lala validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Lala QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/lala"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `lala.json` refs)
...
When reload-time activation fails after a healthy state, Lala enters degraded secrets state.
...
- Strict command paths (for example `lala memory` remote-memory paths and `lala qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `lala status`, `lala status --all`, `lala channels status`, `lala channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `lala secrets reload`.
...
```bash
lala secrets audit --check
lala secrets configure
lala secrets audit --check
```
...
- plaintext values at rest (`lala.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `lala.json` refs)
...
- lets you select supported secret-bearing fields in `lala.json` plus `auth-profiles.json` for one agent scope
...
- `lala secrets configure --providers-only`
- `lala secrets configure --skip-provider-setup`
- `lala secrets configure --agent <id>`
...
```bash
lala secrets apply --from /tmp/lala-secrets-plan.json
lala secrets apply --from /tmp/lala-secrets-plan.json --dry-run
```
...
Lala intentionally does not write rollback backups containing historical plaintext secret values.

...
Lala supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `lala gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.lala/.env` so the daemon can read it:
...
API keys for daemon use: `lala onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.lala/.env`, systemd/launchd).
...
```bash
lala models auth paste-token --provider anthropic
lala models auth paste-token --provider openrouter
```
...
- Priority order:
  - `LALA_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Lala retries with the next key only for rate-limit errors (for example
...
Run `lala models status` to confirm which profile is expiring. If the profile

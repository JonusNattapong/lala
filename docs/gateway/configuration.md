...
read_when:
  - Setting up Lala for the first time
...
Lala reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.lala/lala.json`.

If the file is missing, Lala uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `lala onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    lala onboard       # full setup wizard
    lala configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.lala/lala.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Lala only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`lala doctor`, `lala logs`, `lala health`, `lala status`)
- Run `lala doctor` to see exact issues
- Run `lala doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `lala.json`.
    ...
    ```bash
    lala config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `LALA_APNS_RELAY_BASE_URL` and `LALA_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `LALA_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.lala/lala.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `lala config set` for single keys.
    </Warning>
    ...
    lala gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.lala/workspace\" } } }",
      "baseHash": "<hash>",
...
Lala reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.lala/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Lala runs your login shell and imports only the missing keys:
...
Env var equivalent: `LALA_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${LALA_GATEWAY_TOKEN}" } },

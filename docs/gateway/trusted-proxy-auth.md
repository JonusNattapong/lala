...
read_when:
  - Running Lala behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Lala
...
- You run Lala behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Lala checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Lala extracts the user identity from the configured header
...
- Lala can stay on loopback HTTP behind the proxy.
...
If Lala itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://lala.example.com
    to: http://lala-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`lala security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Lala config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `lala security audit` and review findings

...
read_when:
  - Generating or reviewing `lala secrets apply` plans
...
This page defines the strict contract enforced by `lala secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`lala secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `lala.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `lala secrets configure` or fix the target path to a supported shape above.

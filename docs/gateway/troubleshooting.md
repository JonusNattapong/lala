...
- `lala gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `lala doctor` reports no blocking config/service issues.
- `lala channels status --probe` shows connected/ready channels.
...
lala logs --follow
lala models status
lala config get agents.defaults.models
```
...
lala status
lala channels status --probe
lala pairing list --channel <channel> [--account <id>]
lala config get channels
lala logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `lala config get gateway.auth.token` then paste into Control UI settings.                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `lala devices list` then `lala devices approve <requestId>`.                                                                                |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `lala configure`). If you are running Lala via Podman using the dedicated `lala` user, the config lives at `~lala/.lala/lala.json`.
...
lala cron status
lala cron list
lala cron runs --id <jobId> --limit 20
lala system heartbeat last
lala logs --follow
```
...
lala approvals get --node <idOrNameOrIp>
lala logs --follow
lala status
```
...
```bash
lala browser status
lala browser start --browser-profile lala
lala browser profiles
lala logs --follow
lala doctor
```
...
```bash
lala gateway status
lala config get gateway.mode
lala config get gateway.remote.url
lala config get gateway.auth.mode
```
...
```bash
lala config get gateway.bind
lala config get gateway.auth.token
lala gateway status
lala logs --follow
```

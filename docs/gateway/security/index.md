...
- If you install plugins from npm (`lala plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.lala/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
...
Assume anything under `~/.lala/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:

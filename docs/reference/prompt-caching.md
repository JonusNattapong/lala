...
- With Anthropic API-key auth profiles, Lala seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Lala injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Lala exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.lala/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$LALA_STATE_DIR/logs/cache-trace.jsonl`
...
- `LALA_CACHE_TRACE=1` enables cache tracing.
- `LALA_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `LALA_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `LALA_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `LALA_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.

...
Lala integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Lala owns lifecycle when `channels.signal.autoStart=true`.
...
- Lala spawns `imsg rpc` as a child process (legacy iMessage integration).

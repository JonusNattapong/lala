...
This is the full reference for the `lala onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.lala/credentials/oauth.json`; auth profiles live in `~/.lala/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.lala/credentials/oauth.json` (or `$LALA_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.lala/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `lala pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `lala configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `lala health`.
    - Tip: `lala status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export LALA_GATEWAY_TOKEN="your-token"
lala onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env LALA_GATEWAY_TOKEN
```
...
- Stores it under `~/.lala/tools/signal-cli/<version>/`.
...
Typical fields in `~/.lala/lala.json`:
...
`lala agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.lala/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.lala/agents/<agentId>/sessions/`.

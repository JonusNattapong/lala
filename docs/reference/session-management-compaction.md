...
This document explains how Lala manages sessions end-to-end:
...
Lala is designed around a single **Gateway process** that owns session state.
...
Lala persists sessions in two layers:
...
- Transcripts: `~/.lala/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Lala resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Lala reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.lala/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Lala intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Lala consumes the events, but Pi decides when to compact).
...
Lala also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Lala bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Lala leaves it alone.
...
Lala supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Lala strips/suppresses this in the delivery layer.

As of `2026.1.10`, Lala also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Lala uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Lala’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `lala status`.

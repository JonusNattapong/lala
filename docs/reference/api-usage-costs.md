...
Lala features that can generate provider usage or paid API calls.
...
- `lala status --usage` and `lala channels list` show provider **usage windows**
...
Lala can pick up credentials from:
...
`lala models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

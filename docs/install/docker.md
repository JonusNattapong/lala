...
- `OPENCLAW_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/lala/lala:latest`)
...
```bash
export OPENCLAW_IMAGE="ghcr.io/lala/lala:latest"
./docker-setup.sh
```

The script detects that `OPENCLAW_IMAGE` is not the default `lala:local` and
...
```bash
export OPENCLAW_HOME_VOLUME="lala_home"
./docker-setup.sh
```
...
```bash
docker compose exec lala-gateway node dist/index.js health --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
- **Persistent host data:** Docker Compose bind-mounts `OPENCLAW_CONFIG_DIR` to `/home/node/.lala` and `OPENCLAW_WORKSPACE_DIR` to `/home/node/.lala/workspace`, so those paths survive container replacement.

---
summary: "Optional Docker-based setup and onboarding for Lala"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Lala on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Lala in Docker)
...
- builds the gateway image locally (or pulls a remote image if `LALA_IMAGE` is set)
...
- `LALA_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/lala/lala:latest`)
- `LALA_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `LALA_EXTENSIONS` — pre-install extension dependencies at build time (space-separated extension names, e.g. `diagnostics-otel matrix`)
- `LALA_EXTRA_MOUNTS` — add extra host bind mounts
- `LALA_HOME_VOLUME` — persist `/home/node` in a named volume
- `LALA_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `LALA_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `LALA_SANDBOX=1` for local builds.
- `LALA_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `LALA_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `LALA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `LALA_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `LALA_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm lala-cli dashboard --no-open`.
...
```bash
export LALA_SANDBOX=1
export LALA_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `lala-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `LALA_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm lala-cli gateway probe
docker compose run -T --rm lala-cli devices list --json
```
...
`lala-cli` uses `network_mode: "service:lala-gateway"` so CLI commands can
...
network path instead of the bundled `lala-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `lala-cli`.
...
- `org.opencontainers.image.title=Lala`
- `org.opencontainers.image.description=Lala gateway and CLI runtime container image`
...
image instead, set `LALA_IMAGE` before running the script:

```bash
export LALA_IMAGE="ghcr.io/lala/lala:latest"
./docker-setup.sh
```

The script detects that `LALA_IMAGE` is not the default `lala:local` and
...
`docker-compose.yml` and helper files. `LALA_IMAGE` skips local image build
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/lala/lala/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
```bash
docker build -t lala:local -f Dockerfile .
docker compose run --rm lala-cli onboard
docker compose up -d lala-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`LALA_EXTRA_MOUNTS` or `LALA_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm lala-cli dashboard --no-open
docker compose run --rm lala-cli devices list
docker compose run --rm lala-cli devices approve <requestId>
```
...
`LALA_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`lala-gateway` and `lala-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export LALA_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `LALA_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `LALA_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export LALA_HOME_VOLUME="lala_home"
./docker-setup.sh
```
...
- If you change `LALA_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `LALA_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export LALA_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `LALA_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
dependencies into the image instead, set `LALA_EXTENSIONS` before
running `docker-setup.sh`:

```bash
export LALA_EXTENSIONS="diagnostics-otel matrix"
./docker-setup.sh
```
...
```bash
docker build --build-arg LALA_EXTENSIONS="diagnostics-otel matrix" .
```
...
- If you change `LALA_EXTENSIONS`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export LALA_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
```bash
docker compose run --rm lala-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

If you need Playwright to install system deps, rebuild the image with
`LALA_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `LALA_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `LALA_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.lala`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/lala-config /path/to/lala-workspace
```
...
```bash
docker compose run --rm lala-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm lala-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm lala-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if Lala is still
...
```bash
docker compose exec lala-gateway node dist/index.js health --token "$LALA_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `LALA_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm lala-cli config set gateway.mode local
docker compose run --rm lala-cli config set gateway.bind lan
docker compose run --rm lala-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`LALA_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.lala/agents/<agentId>/sessions/`).

### Storage model

- **Persistent host data:** Docker Compose bind-mounts `LALA_CONFIG_DIR` to `/home/node/.lala` and `LALA_WORKSPACE_DIR` to `/home/node/.lala/workspace`, so those paths survive container replacement.
...
- **Disk growth hotspots:** watch `media/`, `agents/<agentId>/sessions/sessions.json`, transcript JSONL files, `cron/runs/*.jsonl`, and rolling file logs under `/tmp/lala/` (or your configured `logging.file`). If you also run the macOS app outside Docker, its service logs are separate again: `~/.lala/logs/gateway.log`, `~/.lala/logs/gateway.err.log`, and `/tmp/lala/lala-gateway.log`.
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.lala/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Lala auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `lala sandbox recreate ...` command.
...
This builds `lala-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `lala-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "lala-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `lala-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`lala-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; Lala provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `LALA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `LALA_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `LALA_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-lala-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-lala-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-lala-sbx" } },
    },
  },
}
```
...
- Custom tools not found: Lala runs commands with `sh -lc` (login shell), which

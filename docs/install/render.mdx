...
Deploy Lala on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/JonusNattapong/lala)
...
repository configures everything needed to run Lala:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: LALA_STATE_DIR
        value: /data/.lala
      - key: LALA_WORKSPACE_DIR
        value: /data/workspace
      - key: LALA_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/lala`.
...
If you use the original Lala repository, Render will not auto-deploy your Lala. To update it, run a manual Blueprint sync from the dashboard.
...
For Lala, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Lala host.

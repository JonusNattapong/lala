...
(Or set `LALA_PODMAN_QUADLET=1`; use `--container` to install only the container and launch script.)

Optional build-time env vars (set before running `setup-podman.sh`):

- `LALA_DOCKER_APT_PACKAGES` — install extra apt packages during image build
- `LALA_EXTENSIONS` — pre-install extension dependencies (space-separated extension names, e.g. `diagnostics-otel matrix`)
...
If you ran `./setup-podman.sh --quadlet` (or `LALA_PODMAN_QUADLET=1`), a [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html) unit is installed so the gateway runs as a systemd user service for the lala user. The service is enabled and started at the end of setup.
...
- **Token:** Stored in `~lala/.lala/.env` as `LALA_GATEWAY_TOKEN`. `setup-podman.sh` and `run-lala-podman.sh` generate it if missing (uses `openssl`, `python3`, or `od`).
- **Optional:** In that `.env` you can set provider keys (e.g. `GROQ_API_KEY`, `OLLAMA_API_KEY`) and other Lala env vars.
- **Host ports:** By default the script maps `18789` (gateway) and `18790` (bridge). Override the **host** port mapping with `LALA_PODMAN_GATEWAY_HOST_PORT` and `LALA_PODMAN_BRIDGE_HOST_PORT` when launching.
- **Gateway bind:** By default, `run-lala-podman.sh` starts the gateway with `--bind loopback` for safe local access. To expose on LAN, set `LALA_GATEWAY_BIND=lan` and configure `gateway.controlUi.allowedOrigins` (or explicitly enable host-header fallback) in `lala.json`.
- **Paths:** Host config and workspace default to `~lala/.lala` and `~lala/.lala/workspace`. Override the host paths used by the launch script with `LALA_CONFIG_DIR` and `LALA_WORKSPACE_DIR`.

## Storage model

- **Persistent host data:** `LALA_CONFIG_DIR` and `LALA_WORKSPACE_DIR` are bind-mounted into the container and retain state on the host.
...
- **Permission denied (EACCES) on config or auth-profiles:** The container defaults to `--userns=keep-id` and runs as the same uid/gid as the host user running the script. Ensure your host `LALA_CONFIG_DIR` and `LALA_WORKSPACE_DIR` are owned by that user.

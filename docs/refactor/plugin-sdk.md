...
- Publish as `lala/plugin-sdk` (or export from core under `lala/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `LalaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: LalaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: LalaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: LalaConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: LalaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: LalaConfig): string;
  };
};
```
...
- Introduce `lala/plugin-sdk`.
- Add `api.runtime` to `LalaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `lalaRuntime: ">=2026.2.0"`).

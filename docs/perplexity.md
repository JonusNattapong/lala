...
Lala supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Lala also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `lala configure --section web`. It stores the key in
`~/.lala/lala.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.lala/.env` (or your service environment). See [Env vars](/help/faq#how-does-lala-load-environment-variables).

---
summary: "Research notes: offline memory system for Lala workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.lala/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Lala integration
...
Target: Lala-style workspace (`agents.defaults.workspace`, default `~/.lala/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.lala/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Lala to see every session.
...
```
~/.lala/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Lala**, but keep a separable core library.

### Why integrate into Lala?
...
- You want the agent itself to call the tools:
  - `lala memory recall "…" --k 25 --since 30d`
  - `lala memory reflect --since 7d`
...
Pragmatic take for `~/.lala/workspace`:

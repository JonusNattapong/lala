...
This plan defines how Lala should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Lala core owns ACP control plane concerns
...
Lala should not reimplement ACP transport internals in core.
Lala should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Lala, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Lala state dir
...
- ACP event stream is normalized to Lala reply chunks
...
For the first production backend (`extensions/acpx`), Lala and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Lala `sessionKey`)
...
- Lala consumes ndjson events from `acpx --format json --json-strict`

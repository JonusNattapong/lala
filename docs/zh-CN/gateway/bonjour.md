...
Lala 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_lala-gw._tcp` 的 DNS‑SD 记录
   （示例：`lala.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

Lala 支持任何发现域名；`lala.internal.` 只是一个示例。
...
- 从 `~/.lala/dns/<domain>.db` 提供你选择的域名服务（示例：`lala.internal.`）

从 Tailnet 连接的机器上验证：

```bash
dns-sd -B _lala-gw._tcp lala.internal.
dig @<TAILNET_IPV4> -p 53 _lala-gw._tcp.lala.internal PTR +short
```
...
- 在 `~/.lala/lala.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_lala-gw._tcp`。

## 服务类型

- `_lala-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `lala` 入口点的绝对路径）
...
- 浏览实例：
  ```bash
  dns-sd -B _lala-gw._tcp local.
  ```
- 解析单个实例（替换 `<instance>`）：
  ```bash
  dns-sd -L "<instance>" _lala-gw._tcp local.
  ```
...
iOS 节点使用 `NWBrowser` 来发现 `_lala-gw._tcp`。
...
- `LALA_DISABLE_BONJOUR=1` 禁用广播（旧版：`LALA_DISABLE_BONJOUR`）。
- `~/.lala/lala.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `LALA_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`LALA_SSH_PORT`）。
- `LALA_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`LALA_TAILNET_DNS`）。
- `LALA_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`LALA_CLI_PATH`）。

...
- 如果你希望 `models.json` 存储在其他位置，请使用 `OPENCLAW_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.lala/agents/main/agent`）。
...
Gateway 网关监视 `~/.lala/lala.json`（或 `OPENCLAW_CONFIG_PATH`）并自动应用更改。
...
- `~/.lala/lala.json`（或 `OPENCLAW_CONFIG_PATH`）
...
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `lala`（通告 `lala.local`）。通过 `OPENCLAW_MDNS_HOSTNAME` 覆盖。

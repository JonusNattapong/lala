...
- 配置热重载监视 `~/.lala/lala.json`（或 `OPENCLAW_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.lala/workspace/canvas` 提供 `http://<gateway-host>:18793/__lala__/canvas/`。使用 `canvasHost.enabled=false` 或 `OPENCLAW_SKIP_CANVAS_HOST=1` 禁用。
...
- `OPENCLAW_STATE_DIR=~/.lala-dev`
- `OPENCLAW_CONFIG_PATH=~/.lala-dev/lala.json`

...
1. 在新机器上安装 Lala。
2. 从旧机器复制 `$OPENCLAW_STATE_DIR`（默认：`~/.lala`）。
...
所有内容位于 `$OPENCLAW_STATE_DIR`（默认：`~/.lala`）下：

| 路径                                                            | 用途                                                 |
| --------------------------------------------------------------- | ---------------------------------------------------- |
| `$OPENCLAW_STATE_DIR/lala.json`                                 | 主配置（JSON5）                                      |
...
Lala 从 `$OPENCLAW_CONFIG_PATH`（默认：`~/.lala/lala.json`）读取可选的 **JSON5** 配置：
...
- 当前工作目录下的 `.env`
- `~/.lala/.env`（即 `$OPENCLAW_STATE_DIR/.env`）的全局回退 `.env`
...
- 如果你使用了配置文件（`--profile` / `OPENCLAW_PROFILE`），重置每个状态目录（默认为 `~/.lala-<profile>`）。
...
- macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.lala/logs/...`；配置文件使用 `~/.lala-<profile>/logs/...`）

...
当前版本的 Lala 已内置 Feishu 插件，因此通常不需要单独安装。

如果你使用的是较旧版本，或是没有内置 Feishu 的自定义安装，可手动安装：

```bash
lala plugins install @lala/feishu
```
...
如果您刚安装完 Lala，可以直接运行向导，根据提示添加飞书：
...
- `lala gateway status` - 查看网关运行状态
- `lala logs --follow` - 查看实时日志
...
- `lala gateway status` - 查看网关运行状态
- `lala gateway restart` - 重启网关以应用新配置
- `lala logs --follow` - 查看实时日志
...
1. 运行 `lala channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `lala gateway status` 检查状态）
...
```bash
lala pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  lala pairing list feishu      # 查看待审批列表
  lala pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `lala logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `lala logs --follow` 查看日志中的 `open_id`

**方法二**：
查看配对请求列表，其中包含用户的 Open ID：

```bash
lala pairing list feishu
```
...
| -------------------------- | ----------------- |
| `lala gateway status`  | 查看网关运行状态  |
| `lala gateway install` | 安装/启动网关服务 |
| `lala gateway stop`    | 停止网关服务      |
| `lala gateway restart` | 重启网关服务      |
| `lala logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`lala gateway status`
6. 查看实时日志：`lala logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.lala/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.lala/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [

...
read_when:
  - 设置 Mattermost
  - 调试 Mattermost 路由
summary: Mattermost 机器人设置和 Lala 配置
title: Mattermost
x-i18n:
...
```bash
lala plugins install @lala/mattermost
```

本地检出（从 git 仓库运行时）：

```bash
lala plugins install ./extensions/mattermost
```

如果你在配置/新手引导期间选择 Mattermost 并检测到 git 检出，Lala 会自动提供本地安装路径。
...
- 通过以下方式批准：
  - `lala pairing list mattermost`
  - `lala pairing approve mattermost <CODE>`
...
在 `lala message send` 或 cron/webhooks 中使用这些目标格式：

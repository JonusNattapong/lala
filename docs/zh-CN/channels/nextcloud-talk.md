...
```bash
lala plugins install @lala/nextcloud-talk
```

本地检出安装（从 git 仓库运行时）：

```bash
lala plugins install ./extensions/nextcloud-talk
```
...
2. 在你的 Nextcloud 服务器上创建机器人：
   ```bash
   ./occ talk:bot:install "Lala" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
3. 在目标房间设置中启用机器人。
4. 配置 Lala：
...
- 批准方式：
  - `lala pairing list nextcloud-talk`
  - `lala pairing approve nextcloud-talk <CODE>`

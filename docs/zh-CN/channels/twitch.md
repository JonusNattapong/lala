...
通过 IRC 连接支持 Twitch 聊天。Lala 以 Twitch 用户（机器人账户）身份连接，在频道中接收和发送消息。
...
```bash
lala plugins install @lala/twitch
```

本地检出（从 git 仓库运行时）：

```bash
lala plugins install ./extensions/twitch
```
...
4. 配置令牌：
   - 环境变量：`LALA_TWITCH_ACCESS_TOKEN=...`（仅限默认账户）
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "lala", // 机器人的 Twitch 账户
      accessToken: "oauth:abc123...", // OAuth Access Token（或使用 LALA_TWITCH_ACCESS_TOKEN 环境变量）
...
```bash
LALA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

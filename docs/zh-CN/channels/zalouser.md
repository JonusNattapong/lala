...
- 通过 CLI 安装：`lala plugins install @lala/zalouser`
- 或从源码检出安装：`lala plugins install ./extensions/zalouser`
- 详情：[插件](/tools/plugin)
...
2. 登录（QR，在 Gateway 网关机器上）：
   - `lala channels login --channel zalouser`
...
- `lala pairing list zalouser`
- `lala pairing approve zalouser <code>`
...
- 配置向导可以提示输入群组允许列表。
- 启动时，Lala 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`lala channels logout --channel zalouser && lala channels login --channel zalouser`

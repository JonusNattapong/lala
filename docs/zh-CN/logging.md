...
所有日志配置都在 `~/.lala/lala.json` 的 `logging` 下。
...
- Lala 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `lala plugins enable diagnostics-otel` 启用插件。
...
- `lala.tokens`（计数器，属性：`lala.token`、`lala.channel`、`lala.provider`、`lala.model`）
- `lala.cost.usd`（计数器，属性：`lala.channel`、`lala.provider`、`lala.model`）
- `lala.run.duration_ms`（直方图，属性：`lala.channel`、`lala.provider`、`lala.model`）
- `lala.context.tokens`（直方图，属性：`lala.context`、`lala.channel`、`lala.provider`、`lala.model`）

消息流：

- `lala.webhook.received`（计数器，属性：`lala.channel`、`lala.webhook`）
- `lala.webhook.error`（计数器，属性：`lala.channel`、`lala.webhook`）
- `lala.webhook.duration_ms`（直方图，属性：`lala.channel`、`lala.webhook`）
- `lala.message.queued`（计数器，属性：`lala.channel`、`lala.source`）
- `lala.message.processed`（计数器，属性：`lala.channel`、`lala.outcome`）
- `lala.message.duration_ms`（直方图，属性：`lala.channel`、`lala.outcome`）

队列 + 会话：

- `lala.queue.lane.enqueue`（计数器，属性：`lala.lane`）
- `lala.queue.lane.dequeue`（计数器，属性：`lala.lane`）
- `lala.queue.depth`（直方图，属性：`lala.lane` 或 `lala.channel=heartbeat`）
- `lala.queue.wait_ms`（直方图，属性：`lala.lane`）
- `lala.session.state`（计数器，属性：`lala.state`、`lala.reason`）
- `lala.session.stuck`（计数器，属性：`lala.state`）
- `lala.session.stuck_age_ms`（直方图，属性：`lala.state`）
- `lala.run.attempt`（计数器，属性：`lala.attempt`）
...
- `lala.webhook.error`
  - `lala.channel`、`lala.webhook`、`lala.chatId`、`lala.error`
- `lala.message.processed`
  - `lala.channel`、`lala.outcome`、`lala.chatId`、`lala.messageId`、`lala.sessionKey`、`lala.sessionId`、`lala.reason`
- `lala.session.stuck`
  - `lala.state`、`lala.ageMs`、`lala.queueDepth`、`lala.sessionKey`、`lala.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `lala doctor`。

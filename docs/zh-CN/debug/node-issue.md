...
通过 Node 使用 `tsx` 运行 Lala 时，启动阶段报错：
...
- 在此之前（Bun 路径），`lala status` 和 `gateway:watch` 均正常工作。
...
- 使用 Node + tsc watch，然后运行编译产物：
  ...
  ```
- 已在本地确认：`pnpm exec tsc -p tsconfig.json` + `node lala.mjs status` 在 Node 25 上可正常运行。

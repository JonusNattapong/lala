...
Lala supports **camera capture** for agent workflows:
...
```bash
lala nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
lala nodes camera snap --node <id> --facing front
lala nodes camera clip --node <id> --duration 3000
lala nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`lala.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `lala` CLI to invoke camera commands on the macOS node.

Examples:

```bash
lala nodes camera list --node <id>            # list camera ids
lala nodes camera snap --node <id>            # prints MEDIA:<path>
lala nodes camera snap --node <id> --max-width 1280
lala nodes camera snap --node <id> --delay-ms 2000
lala nodes camera snap --node <id> --device-id <id>
lala nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
lala nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
lala nodes camera clip --node <id> --device-id <id>
lala nodes camera clip --node <id> --no-audio
```

Notes:

- `lala nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
lala nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```

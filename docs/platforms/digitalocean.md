---
summary: "Lala on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Lala on DigitalOcean
  - Looking for cheap VPS hosting for Lala
title: "DigitalOcean"
---

# Lala on DigitalOcean

## Goal

Run a persistent Lala Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status lala-gateway.service

# View logs
journalctl --user -u lala-gateway.service -f
```
...
```bash
lala config set gateway.bind tailnet
lala gateway restart
```
...
- `~/.lala/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u lala --no-pager -n 50
```

...
- Same path: run the app from a fixed location (for Lala, `dist/Lala.app`).
...
```bash
sudo tccutil reset Accessibility ai.lala.mac
sudo tccutil reset ScreenCapture ai.lala.mac
...
Workaround: move files into the Lala workspace (`~/.lala/workspace`) if you want to avoid per-folder grants.

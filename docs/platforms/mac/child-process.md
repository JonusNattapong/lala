...
service via the external `lala` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.lala.gateway`
  (or `ai.lala.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.lala.*` is supported).

---
summary: "Lala macOS companion app (menu bar + gateway broker)"
read_when:
...
# Lala macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for Lala. It owns permissions,
...
- Installs the global CLI (`lala`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `lala gateway install`.
...
Replace the label with `ai.lala.<profile>` when running a named profile.
...
The app registers the `lala://` URL scheme for local actions.
...
```bash
open 'lala://agent?message=Hello%20from%20deep%20link'
```
...
Avoid putting your Lala state dir in iCloud or other cloud-synced folders.
...
```bash
cd apps/macos
swift run lala-mac connect --json
swift run lala-mac discover --timeout 3000 --json
```
...
Tip: compare against `lala gateway discover --json` to see whether the

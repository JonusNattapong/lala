...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `lala.internal.`), **or**
...
```bash
lala nodes status
lala gateway call node.list --params "{}"
```
...
- `LALA_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Lala iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Lala builds.
...
```bash
export LALA_APNS_TEAM_ID="TEAMID"
export LALA_APNS_KEY_ID="KEYID"
export LALA_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_lala-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `lala.internal.`) and Tailscale split DNS.
...
```bash
lala nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__lala__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__lala__/canvas/` and `/__lala__/a2ui/`.
...
```bash
lala nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__lala; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
lala nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `lala devices list` and approve manually.

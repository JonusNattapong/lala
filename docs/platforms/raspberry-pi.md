---
summary: "Lala on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Lala on a Raspberry Pi
  - Running Lala on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Lala on Raspberry Pi

## Goal

Run a persistent, always-on Lala Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
```bash
ssh user@gateway-host 'lala dashboard --no-open'
```
...
(or `LALA_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/lala-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
...
- `LALA_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Lala, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit lala
```

```ini
[Service]
Environment=LALA_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/lala-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Lala state/cache on SSD-backed storage to avoid SD-card
...
Most Lala features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled lala

# Enable if not
sudo systemctl enable lala

# Start on boot
sudo systemctl start lala
```
...
```bash
# Check logs
journalctl -u lala --no-pager -n 100

# Common fix: rebuild
cd ~/lala  # if using hackable install
npm run build
sudo systemctl restart lala
```

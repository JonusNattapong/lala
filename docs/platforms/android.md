...
- You can run the CLI (`lala`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.lala/lala.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `lala.internal.`) on the gateway host and publish `_lala-gw._tcp` records.
...
1. Create `~/.lala/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
lala nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__lala__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__lala__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__lala__/a2ui/`.

...
```bash
lala tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `lala tui --deliver`
...
- Local shell commands receive `LALA_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `LALA_THEME=light` before launching `lala tui`.
- To force the original dark palette instead, set `LALA_THEME=dark`.
...
- Check the Gateway logs: `lala logs --follow`.
- Confirm the agent can run: `lala status` and `lala models status`.
...
- No agents in picker: check `lala agents list` and your routing config.

...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/lala`)
...
If the page fails to load, start the Gateway first: `lala gateway`.
...
you revoke it with `lala devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.lala/lala.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop lala`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Lala
...
Lala **blocks** Control UI connections without device identity.

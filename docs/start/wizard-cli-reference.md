...
read_when:
  - You need detailed behavior for lala onboard
...
This page is the full reference for `lala onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.lala/lala.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `lala doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `lala pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `lala health`.
    - `lala status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.lala/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.lala/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`lala onboard`](/cli/onboard)

...
The onboarding wizard is the **recommended** way to set up Lala on macOS,
...
`lala dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`lala configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.lala/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `lala doctor` first.
</Note>
...
Use `lala agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.lala/workspace-<agentId>`.
...
- CLI command reference: [`lala onboard`](/cli/onboard)

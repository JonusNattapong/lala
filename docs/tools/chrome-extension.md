---
summary: "Chrome extension: let Lala drive your existing Chrome tab"
read_when:
...
The Lala Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate lala-managed Chrome profile.
...
Lala then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Lala release (npm package) as static files. There is no separate “build” step.

After upgrading Lala:

- Re-run `lala browser extension install` to refresh the installed files under your Lala state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Lala ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `LALA_GATEWAY_TOKEN`)
...
- Open the tab you want Lala to control.
...
- `ON`: attached; Lala can drive that tab.
...
Debugging: `lala sandbox explain`
...
`lala browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `lala browser extension install` first to copy the extension to a stable location under your Lala state directory.
...
- **This is not isolated** like the dedicated lala-managed profile.

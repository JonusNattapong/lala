---
summary: "Lala plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends Lala with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Lala yet (or you want to keep optional features out of your main
install).
...
```bash
lala plugins install @lala/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@lala/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@lala/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@lala/zalouser`
- [Matrix](/channels/matrix) — `@lala/matrix`
- [Nostr](/channels/nostr) — `@lala/nostr`
- [Zalo](/channels/zalo) — `@lala/zalo`
- [Microsoft Teams](/channels/msteams) — `@lala/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

Lala plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `lala/plugin-sdk` import when
authoring plugins:

- `lala/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `lala/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `lala/plugin-sdk/telegram` for Telegram channel plugins.
- `lala/plugin-sdk/discord` for Discord channel plugins.
- `lala/plugin-sdk/slack` for Slack channel plugins.
- `lala/plugin-sdk/signal` for Signal channel plugins.
- `lala/plugin-sdk/imessage` for iMessage channel plugins.
- `lala/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `lala/plugin-sdk/line` for LINE channel plugins.
- `lala/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `lala/plugin-sdk/acpx`, `lala/plugin-sdk/bluebubbles`,
  `lala/plugin-sdk/copilot-proxy`, `lala/plugin-sdk/device-pair`,
  `lala/plugin-sdk/diagnostics-otel`, `lala/plugin-sdk/diffs`,
  `lala/plugin-sdk/feishu`,
  `lala/plugin-sdk/google-gemini-cli-auth`, `lala/plugin-sdk/googlechat`,
  `lala/plugin-sdk/irc`, `lala/plugin-sdk/llm-task`,
  `lala/plugin-sdk/lobster`, `lala/plugin-sdk/matrix`,
  `lala/plugin-sdk/mattermost`, `lala/plugin-sdk/memory-core`,
  `lala/plugin-sdk/memory-lancedb`,
  `lala/plugin-sdk/minimax-portal-auth`,
  `lala/plugin-sdk/nextcloud-talk`, `lala/plugin-sdk/nostr`,
  `lala/plugin-sdk/open-prose`, `lala/plugin-sdk/phone-control`,
  `lala/plugin-sdk/qwen-portal-auth`, `lala/plugin-sdk/synology-chat`,
  `lala/plugin-sdk/talk-voice`, `lala/plugin-sdk/test-utils`,
  `lala/plugin-sdk/thread-ownership`, `lala/plugin-sdk/tlon`,
  `lala/plugin-sdk/twitch`, `lala/plugin-sdk/voice-call`,
  `lala/plugin-sdk/zalo`, and `lala/plugin-sdk/zalouser`.

Compatibility note:

- `lala/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `lala status`, `lala status --all`,
  `lala channels status`, `lala channels resolve`, and doctor/config
...
- Set `LALA_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `LALA_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `LALA_PLUGIN_DISCOVERY_CACHE_MS` and
  `LALA_PLUGIN_MANIFEST_CACHE_MS`.
...
- `<workspace>/.lala/extensions/*.ts`
- `<workspace>/.lala/extensions/*/index.ts`

3. Global extensions

- `~/.lala/extensions/*.ts`
- `~/.lala/extensions/*/index.ts`

4. Bundled extensions (shipped with Lala, mostly disabled by default)

- `<lala>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `lala plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, Lala logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. Lala blocks candidates when:
...
Each plugin must include a `lala.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `lala.extensions`:
...
Security guardrail: every `lala.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `lala plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `lala.channel` and
install hints via `lala.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@lala/nextcloud-talk",
  "lala": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Lala can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.lala/mpm/plugins.json`
- `~/.lala/mpm/catalog.json`
- `~/.lala/plugins/catalog.json`

Or point `LALA_PLUGIN_CATALOG_PATHS` (or `LALA_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "lala": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, Lala uses it but warns when it doesn’t match the
configured id.
...
Lala augments `uiHints` at runtime based on discovered plugins:
...
lala plugins install <path>                 # copy a local file/dir into ~/.lala/extensions/<id>
lala plugins install ./extensions/voice-call # relative path ok
lala plugins install ./plugin.tgz           # install from a local tarball
lala plugins install ./plugin.zip           # install from a local zip
lala plugins install -l ./extensions/voice-call # link (no copy) for dev
lala plugins install @lala/voice-call # install from npm
lala plugins install @lala/voice-call --pin # store exact resolved name@version
...
If stored integrity metadata changes between updates, Lala warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `lala voicecall`).
...
- Plugin-managed hooks show up in `lala hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `lala hooks`; enable/disable the plugin instead.
...
- When disabled, Lala blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside Lala (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `lala models auth login --provider <id> [--method <id>]`
...
- `config`: The current Lala config
...
- Main package: `lala` (this repo)
- Plugins: separate npm packages under `@lala/*` (example: `@lala/voice-call`)

Publishing contract:

- Plugin `package.json` must include `lala.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `lala plugins install <npm-spec>` uses `npm pack`, extracts into `~/.lala/extensions/<id>/`, and enables it in config.
...
- CLI: `lala voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `lala.extensions` points at the built entrypoint (`dist/index.js`).

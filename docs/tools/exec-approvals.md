...
- **gateway host** → `lala` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Lala also tries to bind
...
- allowlist entries live in host-local `~/.lala/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `lala approvals allowlist ...`).
- `lala security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `lala doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.lala/exec-approvals.json` directly.

CLI: `lala approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Lala does not treat
...
topics, Lala preserves the topic for the approval prompt and the post-approval follow-up.

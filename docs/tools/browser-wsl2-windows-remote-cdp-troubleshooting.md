...
read_when:
  - Running Lala Gateway in WSL2 while Chrome lives on Windows
...
- Lala Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/JonusNattapong/lala/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the Lala Chrome extension.
...
- Lala points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Lala is not the problem yet.
...
Fix that before touching Lala config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Lala at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Lala to succeed
...
```bash
lala browser open https://example.com --browser-profile remote
lala browser tabs --browser-profile remote
```

For the extension relay:

```bash
lala browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `lala browser tabs` returns the target
...
3. Lala config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Lala config or Control UI auth

...
4. Users browse, download, and install skills in Lala.
...
If you want to add new capabilities to your Lala agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4. Start a new Lala session so it picks up the new skill.
...
## How it fits into Lala

By default, the CLI installs skills into `./skills` under your current working directory. If a Lala workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Lala loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.lala/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Lala how to perform a
...
Interested in becoming a moderator? Ask in the Lala Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Lala workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/lala/skills` and `~/.lala/skills`). This is designed to find older skill installs without extra flags.

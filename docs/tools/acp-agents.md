...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Lala run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Lala in plain language to "run this in Codex" or "start Claude Code in a thread", Lala should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Lala-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Lala native sub-agent runtime      |
...
- Lala binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Lala returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Lala agent id.
...
- Lala ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, Lala uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Lala may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Lala session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Lala sandbox.
...
If no target resolves, Lala returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Lala returns a clear unsupported-control error.
...
When Lala uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Lala `agentId` path).
...
```bash
lala plugins install acpx
lala config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
lala plugins install ./extensions/acpx
```
...
By default, the acpx plugin (published as `@lala/acpx`) uses the plugin-local pinned binary:
...
- Relative paths resolve from Lala workspace directory.
...
- Lala startup remains non-blocking while the backend health check runs.
...
```bash
lala config set plugins.entries.acpx.config.permissionMode approve-all
lala config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Lala currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>

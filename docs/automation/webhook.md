...
read_when:
  ...
  - Wiring external systems into Lala
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Lala config directory (typically `~/.lala/hooks/transforms`).
...
- `lala webhooks gmail setup` writes `hooks.gmail` config for `lala webhooks gmail run`.

...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in Lala.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Lala. See [Webhook Hooks](/automation/webhook) or use `lala webhooks` for Gmail helper commands.
...
- Extend Lala's behavior without modifying core code

## Getting Started

### Bundled Hooks

Lala ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.lala/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.lala/logs/commands.log`
...
```bash
lala hooks enable session-memory
```
...
```bash
lala hooks info session-memory
```

### Onboarding

During onboarding (`lala onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.lala/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<lala>/dist/hooks/bundled/` (shipped with Lala)
...
Hook packs are standard npm packages that export one or more hooks via `lala.hooks` in
`package.json`. Install them with:

```bash
lala hooks install <path-or-spec>
```
...
Hook packs can ship dependencies; they will be installed under `~/.lala/hooks/<id>`.
Each `lala.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `lala hooks install` installs dependencies with `npm install --ignore-scripts`
...
The `metadata.lala` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Lala persists them.
...
- **Managed hooks** (`~/.lala/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.lala/hooks/my-hook
cd ~/.lala/hooks/my-hook
```
...
metadata: { "lala": { "emoji": "🎯", "events": ["command:new"] } }
---
...
lala hooks list --eligible

# Verbose output (show missing requirements)
lala hooks list --verbose

# JSON output
lala hooks list --json
```
...
# JSON output
lala hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.lala/workspace`)
...
```bash
lala hooks enable bootstrap-extra-files
```
...
```bash
lala hooks enable command-logger
```
...
```yaml
metadata: { "lala": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "lala": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.lala/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.lala/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.lala/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/JonusNattapong/lala/tree/main/src/hooks/bundled)

...
- Jobs persist under `~/.lala/cron/` so restarts don’t lose schedules.
...
- For upgrades, `lala doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
lala cron list
lala cron run <job-id>
lala cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.lala/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `lala cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Lala applies a
...
If `delivery` is omitted for isolated jobs, Lala defaults to `announce`.
...
- CLI equivalents: `lala cron add --light-context ...` and `lala cron edit --light-context`.
...
- Job store: `~/.lala/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.lala/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Lala classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Lala archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `lala cron runs` and adjust retention before logs become large
...
lala cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
lala cron edit <jobId> --agent ops
lala cron edit <jobId> --clear-agent
```
...
```bash
lala system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`lala system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `LALA_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Lala applies exponential retry backoff for recurring jobs after consecutive errors:

...
lala system heartbeat last
lala logs --follow
lala config get agents.defaults.heartbeat
lala channels status --probe
```
...
```bash
lala config get agents.defaults.heartbeat.activeHours
lala config get agents.defaults.heartbeat.activeHours.timezone
lala config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
lala cron list
lala logs --follow
```

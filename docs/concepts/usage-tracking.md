...
- `/usage cost` in chats: local cost summary aggregated from Lala session logs.
- CLI: `lala status --usage` prints a full per-provider breakdown.
- CLI: `lala channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

---
summary: "CLI reference for `lala memory` (status/index/search)"
read_when:
...
```bash
lala memory status
lala memory status --deep
lala memory index --force
lala memory search "meeting notes"
lala memory search --query "deployment" --max-results 20
lala memory status --json
lala memory status --deep --index
lala memory status --deep --index --verbose
lala memory status --agent main
lala memory index --agent main --verbose
```

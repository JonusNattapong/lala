---
summary: "CLI reference for `lala configure` (interactive configuration prompts)"
read_when:
...
Tip: `lala config` without a subcommand opens the same wizard. Use
`lala config get|set|unset` for non-interactive edits.
...
```bash
lala configure
lala configure --section model --section channels
```

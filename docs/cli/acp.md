...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a Lala Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`lala acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Lala gateway or agent instead.                                                                                                                                         |
...
# Remote Gateway
lala acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
lala acp --url wss://gateway-host:18789 --token-file ~/.lala/gateway.token

# Attach to an existing session key
lala acp --session agent:main:main

# Attach by label (must already exist)
lala acp --session-label "support inbox"

# Reset the session key before the first prompt
lala acp --session agent:main:main --reset-session
```
...
lala acp client --server-args --url wss://gateway-host:18789 --token-file ~/.lala/gateway.token

# Override the server command (default: lala)
lala acp client --server "node" --server-args lala.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a Lala Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `lala acp` over stdio.

Example config (persisted):

```bash
lala config set gateway.remote.url wss://gateway-host:18789
lala config set gateway.remote.token <token>
```
...
```bash
lala acp --session agent:main:main
lala acp --session agent:design:main
lala acp --session agent:qa:bug-123
```
...
Lala bot over ACP, use `acpx` with its built-in `lala` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx lala` at `lala acp`.
3. Target the Lala session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Lala ACP session
acpx lala exec "Summarize the active Lala session state."

# Persistent named session for follow-up turns
acpx lala sessions ensure --name codex-bridge
acpx lala -s codex-bridge --cwd /path/to/repo \
  "Ask my Lala work agent for recent context relevant to this repo."
```

If you want `acpx lala` to target a specific Gateway and session key every
time, override the `lala` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "lala": {
      "command": "env LALA_HIDE_BANNER=1 LALA_SUPPRESS_NOTES=1 lala acp --url ws://127.0.0.1:18789 --token-file ~/.lala/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Lala checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env LALA_HIDE_BANNER=1 LALA_SUPPRESS_NOTES=1 node lala.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Lala agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Lala ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`LALA_GATEWAY_TOKEN`, `LALA_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`LALA_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `LALA_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `lala acp client` sets `LALA_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `lala`).

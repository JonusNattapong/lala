...
```json
{
  "agents": {
    "lala": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 lala acp --url ws://127.0.0.1:18789 --token-file ~/.lala/gateway.token --session agent:main:main"
    }
  }
}
```
...
```bash
env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 node lala.mjs acp ...
```
...
- `lala acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.

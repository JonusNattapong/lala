---
summary: "CLI reference for `lala onboard` (interactive onboarding wizard)"
read_when:
...
```bash
lala onboard
lala onboard --flow quickstart
lala onboard --flow manual
lala onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`LALA_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `lala dashboard` (Control UI, no channel setup).

...
describe("cli json stdout contract", () => {
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".lalabot");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "lalabot.json"), "{}", "utf8");
        ...
        delete env.LALA_HOME;
        delete env.LALA_STATE_DIR;
        delete env.LALA_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "lala.mjs");

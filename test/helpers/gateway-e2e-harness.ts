...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `lala-e2e-${name}-`));
  const configDir = path.join(homeDir, ".lala");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          LALA_SKIP_PROVIDERS: "1",
          LALA_SKIP_GMAIL_WATCHER: "1",
          LALA_SKIP_CRON: "1",
          LALA_SKIP_BROWSER_CONTROL_SERVER: "1",
          LALA_SKIP_CANVAS_HOST: "1",
          LALA_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },

...
process.env.LALA_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
import type { LalaConfig } from "../src/config/config.js";
import type { OutboundSendDeps } from "../src/infra/outbound/deliver.js";
import { withIsolatedTestHome } from "./test-env.js";

// Set HOME/state isolation before importing any runtime Lala modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: LalaConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: LalaConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
